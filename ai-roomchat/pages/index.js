...
function deriveNextActionTiming(action) {
  const fallbackLabel =
    action.timing?.label || action.targetDateDisplay || action.targetDateISO || null;
    ...
    <main className={styles.hero} style={heroStyle}>
      <section className={styles.frame}>
        ...
        <p className={styles.lede}>
          자동 매칭, 프롬프트 기반 전투, 공유 히스토리를 결합한 경쟁 모드 비전을 지금 바로 체험해
          보세요.
        </p>
        ...
        <div className={styles.arenaNotice}>
          ...
          <p>
            빠른 매칭이 필요할 때는 <Link href="/arena/queue">Rank Arena</Link>에서 큐 대기 → 준비
            투표 → 본게임 흐름을 따라 주세요. Arena는 Supabase RPC와 Realtime만으로 방 검색을
            대체하며 기존 방 목록은 비교용으로 남겨 두었습니다.
          </p>
          <p className={styles.arenaNoticeHint}>
            운영 포털에서 타이틀 UI를 조정하더라도 이 안내는 최신 Arena 흐름을 가리키도록
            유지됩니다.
          </p>
        </div>
            ...
            <div className={styles.progressHeader}>
              ...
              >
                ...
                {progressRecency.relativeLabel ? (
                  <span className={styles.progressRelative}>· {progressRecency.relativeLabel}</span>
                ) : null}
              </div>
            </div>
            ...
            {hasNextActions ? (
              <ol className={styles.nextActionsList}>
                {sortedNextActions.map(action => {
                  ...
                  const priorityRank =
                    typeof action.priority?.rank === 'number'
                      ? Math.round(action.priority.rank)
                      : null;
                  const effortBadge = formatPersonDays(action.effort?.personDays);
                  return (
                    <li key={action.order} className={styles.nextActionItem}>
                      <span className={styles.nextActionIndex}>
                        {action.order.toString().padStart(2, '0')}
                      </span>
                          ...
                          <div className={styles.nextActionMeta}>
                            {action.priority?.label ? (
                              ...
                              >
                                ...
                                <span className={styles.nextActionPriorityLabel}>
                                  {action.priority.label}
                                </span>
                              </span>
                            ) : null}
                            ...
                            {action.effort?.label ? (
                              <span className={styles.nextActionEffort}>
                                {effortBadge ? (
                                  <span className={styles.nextActionEffortBadge}>
                                    {effortBadge}
                                  </span>
                                ) : null}
                                <span className={styles.nextActionEffortLabel}>
                                  {action.effort.label}
                                </span>
                              </span>
                            ) : null}
