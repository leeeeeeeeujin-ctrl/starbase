...
async function bucketExists() {
  try {
    ...
    const exists =
      Array.isArray(data) && data.some(b => b.id === STORAGE_BUCKET || b.name === STORAGE_BUCKET);
    return { ok: exists };
  } catch (error) {
  ...
  try {
    ...
    if (error) {
      if (isMissingSupabaseTable(error)) {
        return res
          .status(200)
          .json({ settings: null, meta: { missingTable: true, missingBucket: false } });
      }
...
async function handlePut(req, res) {
  ...
  try {
    ...
    if (!bucketCheck.ok) {
      ...
      return respondError(res, 500, '스토리지 버킷 목록을 가져오지 못했습니다.', {
        supabase: supaMeta,
      });
    }
    ...
    const currentSettings =
      Array.isArray(existingData) && existingData.length > 0 ? existingData[0] : null;
    ...
    if (file) {
      ...
      if (decoded.buffer.length > MAX_FILE_BYTES) {
        return respondError(
          res,
          400,
          '파일 용량이 너무 큽니다. 8MB 이하 이미지를 업로드해주세요.',
          meta
        );
      }
    ...
    return res
      .status(200)
      .json({ settings, meta: { missingTable: false, missingBucket: meta.missingBucket } });
  } catch (error) {
