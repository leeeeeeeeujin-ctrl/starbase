...
function buildMetadata(payload: Record<string, unknown>) {
  ...
  const viewerId = safeString(payload.viewer_id) ?? safeString(payload.viewerId) ?? null;
  ...
  return {
    ...
    metadata: {
      apiKeyPool: {
        ...
        provider: safeString(payload.provider) ?? safeString(payload.api_provider) ?? null,
        poolId: safeString(payload.pool_id) ?? safeString(payload.poolId) ?? null,
        rotationId: safeString(payload.rotation_id) ?? safeString(payload.rotationId) ?? null,
        reason,
        note: safeString(payload.note) ?? null,
        newSample,
        replacedSample,
        viewerId,
        poolSize: safeNumber(payload.pool_size) ?? safeNumber(payload.poolSize) ?? null,
        exhaustedCount:
          safeNumber(payload.exhausted_count) ?? safeNumber(payload.exhaustedCount) ?? null,
        available: safeNumber(payload.available) ?? safeNumber(payload.available_count) ?? null,
