...
export function parseResultAssignments(line, fallbackActors = []) {
  ...
  const assignments = []
  const fallbackName = Array.isArray(fallbackActors) && fallbackActors.length === 1 ? fallbackActors[0] : null
  ...
  segments.forEach((segment) => {
    ...
    const resultRegex = /(승리|승|victory|win|triumph|패배|패|defeat|lose|loss|탈락|퇴장|추방|out|eliminate|banished|퇴출)/i
...
function createEntryFromParticipant(participant, index = 0) {
  ...
  const ownerIdRaw =
    participant?.owner_id ?? participant?.ownerId ?? participant?.ownerID ?? participant?.owner?.id ?? null
...
function ensureRoleRangeMap(roleSettings = {}) {
  ...
  if (Array.isArray(roleSettings)) {
    roleSettings.forEach((entry) => {
      ...
      const min = Number.isFinite(Number(entry.score_delta_min)) ? Number(entry.score_delta_min) : null
      const max = Number.isFinite(Number(entry.score_delta_max)) ? Number(entry.score_delta_max) : null
...
function applyScoreModel(ledger, { brawlEnabled = false } = {}) {
  ...
  ledger.entries.forEach((entry) => {
    ...
    const roleAverage = (() => {
      const scores = entries
        .map((candidate) => (Number.isFinite(candidate.baseScore) ? Number(candidate.baseScore) : null))
        .filter((value) => value != null)
...
export function recordOutcomeLedger(
  ledger,
  { turn, slotIndex = null, resultLine = '', variables = [], actors = [], participantsSnapshot = [], brawlEnabled = false } = {},
) {
  if (!ledger || !turn) {
