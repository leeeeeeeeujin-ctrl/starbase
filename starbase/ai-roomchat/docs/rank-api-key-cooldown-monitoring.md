...
4. **Telemetry 리포트 & 대시보드**
  ...
  - 타임라인 컨트롤에는 CSV/PNG 내보내기 버튼이 붙어 있어 선택한 일·주·월 모드 데이터를 바로 내려받아 주간 보고나 회고 문서에 첨부할 수 있습니다.
...
느낀 점: 일간/주간/월간 토글이 붙으니 감사 이벤트의 단기 변동과 장기 추세를 한 패널에서 비교할 수 있어 분석 속도가 크게 빨라졌습니다.
추가로 필요한 점: 자동 업로드 스크립트까지 연결해 주기적으로 팀 드라이브에 적재되면 공유 흐름이 더 단순해질 것 같습니다.
진행사항: 감사 타임라인에 CSV·PNG 내보내기를 추가하고 API/대시보드/문서를 업데이트해 선택한 모드를 그대로 보고서에 활용할 수 있게 했습니다.
