...
export default function GameListPanel() {
  ...
  useEffect(() => {
    const nextOptions = supportsMetrics
      ? SORTS
      : SORTS.filter(option => !METRIC_SORT_KEYS.has(option.key));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let sel = supabase.from('rank_games').select(selectColumns, { count: 'exact' });
    ...
    <div style={{ display: 'grid', gap: 8 }}>
      ...
      >
        ...
        {!loading && rows.length === 0 && (
          <div style={{ padding: 8, color: '#64748b' }}>검색 결과가 없습니다.</div>
        )}
      </div>
    </div>
  );
