...
export default function AutoMatchProgress({ gameId, mode, initialHeroId, onClose }) {
  ...
  const [confirmationRemaining, setConfirmationRemaining] = useState(CONFIRMATION_WINDOW_SECONDS);
  ...
  const roleName = useMemo(
    () => resolveRoleName(state.lockedRole, state.roles),
  ...
  useEffect(() => {
    ...
    if (state.status === 'matched' && !matchRedirectedRef.current && gameId && mode) {
      ...
      try {
        ...
        if (!response.ok || (payload && payload.ok === false && payload.error)) {
          const message =
            payload?.error ||
            payload?.detail ||
            '전투를 시작하지 못했습니다. 잠시 후 다시 시도해 주세요.';
          if (message === 'missing_user_api_key') {
            setJoinError(
              'AI API 키가 필요합니다. 전투 화면에서 키를 입력한 뒤 다시 시도해 주세요.'
            );
          } else {
    ...
    try {
      ...
      const response = await fetch('/api/rank/start-session', {
        ...
        body: JSON.stringify({
          ...
          turn_timer:
            Number.isFinite(Number(turnTimer)) && Number(turnTimer) > 0
              ? Number(turnTimer)
              : undefined,
        }),
      });
      ...
      if (!response.ok) {
        ...
        const message =
          payload?.error ||
          payload?.detail ||
          '전투를 준비하지 못했습니다. 잠시 후 다시 시도해 주세요.';
        setJoinError(message);
        storeStartMatchMeta(null);
        return;
      }
      ...
      const payload = await response.json().catch(() => ({}));
      if (!payload?.ok) {
        const message = payload?.error || '전투를 준비하지 못했습니다. 잠시 후 다시 시도해 주세요.';
        setJoinError(message);
        storeStartMatchMeta(null);
        return;
      }
  ...
  }, [cancelQueueWithCleanup, gameId, requiresManualConfirmation, router, state.status]);
  ...
  useEffect(() => {
    ...
    queueJoinStartedAtRef.current = null;
  }, [clearConfirmationTimers, confirmationState, startConfirmationCountdown, state.status]);
  ...
  const heroMap = useMemo(() => coerceHeroMap(state.match?.heroMap), [state.match?.heroMap]);

  const relevantAssignments = useMemo(() => {
    const assignments = Array.isArray(state.match?.assignments) ? state.match.assignments : [];
  ...
  return (
    ...
    >
      <div className={styles.baseLayer} aria-hidden={showMatchedOverlay ? 'true' : 'false'}>
        {showBaseSpinner ? <div className={styles.spinner} aria-hidden="true" /> : null}
        <div className={styles.status} role="status">
          <p className={styles.message}>{baseDisplay.title}</p>
          {baseDisplay.detail ? <p className={styles.detail}>{baseDisplay.detail}</p> : null}
              ...
              {assignmentSummary.map(assignment => (
                <div key={assignment.key} className={styles.assignmentItem}>
                  ...
                  {assignment.missing > 0 ? (
                    <p className={styles.assignmentEmpty}>남은 슬롯 {assignment.missing}</p>
                  ) : null}
        ...
        <div className={styles.matchedOverlay} role="status" aria-live="assertive">
          <div className={styles.status}>
            ...
            {confirmationState === 'counting' ? (
              <div className={styles.confirmArea}>
                ...
                <p className={styles.confirmHint}>
                  {CONFIRMATION_WINDOW_SECONDS}초 안에 게임을 시작하지 않으면 매칭이 취소되고 다시
                  매칭이 진행됩니다.
                </p>
              </div>
            ) : null}
            {confirmationState === 'confirmed' ? (
              <div className={styles.confirmArea}>
                <p className={styles.confirmHint}>
                  참가 의사를 확인했습니다. 남은 {confirmationRemaining}초 동안 준비를 마무리해
                  주세요.
                </p>
              </div>
            ) : null}
