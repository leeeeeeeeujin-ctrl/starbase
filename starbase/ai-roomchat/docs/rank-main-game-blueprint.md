...
- **플레이어 히스토리**: 각 플레이어는 자신이 볼 수 있었던 AI 응답을 실시간으로 확인 가능하며, 세션이 끝나면 해당 응답과 자신의 입력을 모아 ‘배틀 로그’로 저장한다.
...
---
느낀 점: 실시간과 비실시간 규칙을 한 문서에 그려보니 동일 프롬프트 그래프를 공유하면서도 모드별로 최소한의 분기만 두고 운영할 수 있겠다는 확신이 들었습니다.
추가로 필요한 점: 난입 큐와 API 키 대체 로직은 서비스 롤 백엔드 설계가 선행돼야 하므로, 매칭/키 관리 모듈의 스펙을 별도로 정의할 필요가 있습니다.
진행사항: 턴 타이머, 프롬프트 규칙, 실시간/비실시간 분기, 난입·히스토리 처리까지 한 흐름으로 엮은 메인 게임 청사진을 작성했습니다.
