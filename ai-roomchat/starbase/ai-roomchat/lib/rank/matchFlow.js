import { readGameMatchData, clearGameMatchData } from '../../modules/rank/matchDataStore';
...
function normalizeSlotTemplate(rawSlotTemplate, snapshot) {
  ...
  if (!base.slots.length && snapshot) {
    const fallbackSlots = sanitizeRoster(
      (snapshot.match?.roleStatus && snapshot.match.roleStatus.slotLayout) ||
        snapshot.match?.slotLayout
    );
    ...
    const fallbackVersion =
      toNumber(snapshot.match?.roleStatus?.version) ??
      toNumber(snapshot.match?.roleStatus?.updatedAt);
