...
function sanitizeAsyncFillSnapshot(snapshot) {
  ...
  const sanitized = {
    mode:
      typeof clone.mode === 'string' ? clone.mode.trim() || REALTIME_MODES.OFF : REALTIME_MODES.OFF,
  ...
  const body = cleanupPayload({
    ...
    match_instance_id:
      typeof matchInstanceId === 'string' && matchInstanceId.trim()
        ...
        : undefined,
    collaborators: Array.isArray(collaborators) && collaborators.length ? collaborators : undefined,
