...
export function buildRuntimePromptFromTurn(session, turn, actorId = session?.actorId) {
  ...
  const resultContract = [
    ...
    'reply만 길게 쓰지 말고, 장면을 보여주기 위해 반드시 segments를 함께 제공한다.',
    'dialogue는 캐릭터 대사, narration은 서술, effect는 짧은 효과 문구, sceneCue는 장소/인물/BGM 전환 카드에 쓴다.',
    'sceneCue는 title, subtitle을 채우고 text는 비워도 된다.',
    'segments는 긴 문단 하나로 쓰지 말고, 한 번에 화면에 보여줄 호흡 단위로 나눈다.',
    'dialogue는 보통 대사 한 줄만 담고, narration도 1~2문장 정도의 짧은 서술만 담는다.',
    '서술, 대사, 효과, 장면 전환은 가능한 한 서로 다른 세그먼트로 나눈다.',
    '예: 서술 1개 -> sceneCue 1개 -> 대사 1개 같은 식으로, 한 세그먼트가 한 역할만 맡게 한다.',
    '긴 장면 설명을 한 세그먼트에 몰아넣지 말고 자연스럽게 여러 세그먼트로 쪼갠다.',
