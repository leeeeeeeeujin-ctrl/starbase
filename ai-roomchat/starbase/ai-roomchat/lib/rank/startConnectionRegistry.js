...
function normaliseEntry(entry) {
  ...
  const slotIndices = normaliseSlotIndices(entry.slotIndices || entry.roleSlots || [], entry.members?.length || 0)
  ...
  assignments.forEach((assignment) => {
    ...
    members.forEach((member, memberIndex) => {
      ...
      entries.push({
        ...
        ownerId,
        viewerId:
          viewerId && normaliseOwnerId(viewerId) === ownerId
            ? normaliseOwnerId(viewerId)
            : ownerId,
        heroId: heroId || null,
        heroName,
        role,
