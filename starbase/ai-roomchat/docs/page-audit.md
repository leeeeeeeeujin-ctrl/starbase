...
- Uses `Home.module.css` for glassmorphism styling, a core feature list, and a blueprint progress board that reads JSON-backed stage status/percentages with a freshness badge that flags stale data to visitors. `npm run refresh:blueprint-progress` regenerates the JSON from the overview doc so the UI stays aligned with docs without manual edits, and `npm run check:blueprint-progress-freshness` (also executed weekly via GitHub Actions) fails when the snapshot is older than the 14일 한계치 so maintainers receive a reminder.【F:pages/index.js†L52-L118】【F:styles/Home.module.css†L1-L214】【F:data/rankBlueprintProgress.json†L1-L35】【F:scripts/refresh-rank-blueprint-progress.js†L1-L94】【F:scripts/check-rank-blueprint-progress-freshness.js†L1-L74】【F:.github/workflows/blueprint-progress-freshness.yml†L1-L20】
