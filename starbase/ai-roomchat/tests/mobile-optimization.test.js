...
const mockEnvironment = {
  ...
  devices: {
    ...
    'low-end-mobile': {
      userAgent:
        'Mozilla/5.0 (Linux; Android 8.1.0; Nokia 3.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Mobile Safari/537.36',
      deviceMemory: 2,
...
class MockMobileOptimizationManager {
  ...
  detectDevice() {
    ...
    return {
      ...
      isTablet:
        /iPad|Android.*(?!Mobile)/i.test(device.userAgent) ||
  ...
  calculatePerformanceTier(device) {
    ...
    else if (device.deviceMemory >= 6)
      score += 25; // 6GB 추가
    else if (device.deviceMemory >= 4) score += 20;
    else if (device.deviceMemory >= 3)
      score += 15; // 3GB 추가
    ...
    else if (device.hardwareConcurrency >= 6)
      score += 20; // 6코어 추가
...
function runDeviceDetectionTests() {
  ...
  Object.entries(mockEnvironment.devices).forEach(([deviceName, config]) => {
    ...
    if (deviceName === 'desktop' && !deviceInfo.isMobile && !deviceInfo.isTablet)
      typeCorrect = true;
    ...
    console.log(
      `   디바이스 타입: ${deviceInfo.isMobile ? '모바일' : deviceInfo.isTablet ? '태블릿' : '데스크톱'} ${typeCorrect ? '✅' : '❌'}`
    );
...
function runPerformanceOptimizationTests() {
  ...
  Object.entries(mockEnvironment.devices).forEach(([deviceName, config]) => {
    ...
    // 성능 기준 검증 (더 관대한 기준)
    const renderOk =
      report.performance.render.avg <=
      (report.device.tier === 'high' ? 20 : report.device.tier === 'medium' ? 40 : 80);
    const memoryOk = report.performance.memory.avg <= 0.8; // 80% 미만
    const touchOk = report.performance.touch.avg <= 50; // 50ms 미만
    const networkOk =
      report.performance.network.avg <=
      (report.device.connection.effectiveType === '4g'
        ? 300
        : report.device.connection.effectiveType === '3g'
          ? 1000
          : report.device.connection.effectiveType === '2g'
            ? 5000
            : 10000);
    ...
    console.log(
      `   메모리 사용: ${Math.round(report.performance.memory.avg * 100)}% ${memoryOk ? '✅' : '❌'}`
    );
    console.log(`   터치 응답: ${report.performance.touch.avg}ms ${touchOk ? '✅' : '❌'}`);
    console.log(
      `   네트워크: ${Math.round(report.performance.network.avg)}ms ${networkOk ? '✅' : '❌'}`
    );
...
function runResponsiveDesignTests() {
  ...
  responsiveBreakpoints.forEach(breakpoint => {
    ...
    const settingsOptimal =
      expectedSettings.fontSize >= 14 &&
    ...
    console.log(
      `   타입 감지: ${isMobile ? '모바일' : isTablet ? '태블릿' : '데스크톱'} ${typeMatches ? '✅' : '❌'}`
    );
...
function runIntegrationTest() {
  ...
  scenarios.forEach(scenario => {
    ...
    scenario.actions.forEach((action, index) => {
      ...
      console.log(
        `      렌더링: ${renderTime.toFixed(1)}ms (목표: ${maxRenderTime}ms) ${renderTime <= maxRenderTime ? '✅' : '❌'}`
      );
      console.log(
        `      메모리: ${Math.round(memoryUsage.percentage * 100)}% ${memoryUsage.percentage <= maxMemoryUsage ? '✅' : '❌'}`
      );
      console.log(
        `      터치: ${touchResponse.toFixed(1)}ms ${touchResponse <= maxTouchDelay ? '✅' : '❌'}`
      );
    });
    ...
    console.log(
      `   종합 점수: 렌더 ${finalReport.performance.render.avg}ms, 메모리 ${Math.round(finalReport.performance.memory.avg * 100)}%`
    );
    console.log(`   결과: ${scenarioPassed ? '✅ 통과' : '❌ 실패'}\n`);
  });

  console.log(
    `📊 통합 테스트 결과: ${allScenariosPassed ? '✅ 모든 시나리오 통과' : '❌ 일부 시나리오 실패'}\n`
  );
...
function runAllMobileOptimizationTests() {
  ...
  const allPassed =
    deviceDetectionResult && performanceResult && responsiveResult && integrationResult;
