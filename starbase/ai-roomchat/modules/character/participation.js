...
function normaliseParticipant(row) {
  ...
  return {
    ...
    win_rate:
      row.win_rate != null && Number.isFinite(Number(row.win_rate)) ? Number(row.win_rate) : null,
    created_at: row.created_at || null,
...
export async function fetchHeroParticipationBundle(heroId, { heroSeed } = {}) {
  ...
  if (heroIds.size) {
    const { data: heroRows, error: heroError } = await withTable(supabase, 'heroes', table =>
      supabase.from(table).select(HERO_LOOKUP_COLUMNS).in('id', Array.from(heroIds))
    );
  ...
  const participations = heroParticipationSlots.map(slot => {
    ...
    const effectiveRole =
      participant?.role || slot.role || (slot.slot_no != null ? `슬롯 ${slot.slot_no}` : '');
