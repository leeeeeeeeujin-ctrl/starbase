...
export function createTurnVoteController({ thresholdRatio = DEFAULT_THRESHOLD_RATIO } = {}) {
  let ratio = Number.isFinite(Number(thresholdRatio))
  ...
  return {
    ...
    syncEligibleOwners(nextEligible = []) {
      const normalized = Array.isArray(nextEligible)
        ? nextEligible.map(value => normalizeOwnerId(value)).filter(value => value !== null)
        : [];
      const unique = Array.from(new Set(normalized));
      const sameLength = unique.length === eligibleOwnerIds.length;
      const sameOrder =
        sameLength && unique.every((value, index) => eligibleOwnerIds[index] === value);
      if (!sameOrder) {
