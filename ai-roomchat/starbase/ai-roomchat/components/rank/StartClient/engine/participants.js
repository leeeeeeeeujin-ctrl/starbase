...
export function buildOwnerRosterSnapshot(participants = []) {
  ...
  map.forEach((participant, ownerId) => {
    const heroIdRaw = participant?.hero?.id ?? participant?.hero_id ?? participant?.heroId ?? null;
    const heroId = heroIdRaw != null && String(heroIdRaw).trim() ? String(heroIdRaw).trim() : null;
    const heroName =
      participant?.hero?.name ??
      participant?.hero_name ??
      participant?.display_name ??
