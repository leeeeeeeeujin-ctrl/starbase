...
---
느낀 점: 난입 턴 타이머 보너스와 히스토리 가시성을 추가하면서, 실시간/비실시간 흐름이 실제 운영 시 더 온화하게 연결될 것 같다는 생각이 들었습니다.
추가로 필요한 점: 난입 큐와 API 키 대체 로직은 서비스 롤 백엔드 설계가 선행돼야 하므로, 매칭/키 관리 모듈의 스펙을 별도로 정의할 필요가 있습니다.
진행사항: 턴 타이머, 프롬프트 규칙, 실시간/비실시간 분기, 난입 보너스·히스토리 처리, 그리고 모듈화된 구현 계획까지 정리한 메인 게임 청사진을 작성했습니다.
