...
## 구현 진행 현황 업데이트
- **진행률**: 65% – Phase 3의 실시간 경고·대역 전환 뼈대를 도입해 참가자 관리와 투표 흐름이 청사진 수준의 안정성을 갖추기 시작했습니다.
- **완료 사항**:
  - `components/rank/StartClient/services/realtimeSessionManager`로 경고 한도, 연속 미응답 추적, 대역 전환 시점을 일관되게 계산하고 스냅샷을 노출하도록 했습니다.
  - `useStartClientEngine`에 실시간 매니저를 통합해 턴 시작 시 자격 목록을 동기화하고, 경고/대역 전환 이벤트를 UI 상태 및 참여자 리스트에 반영했습니다.
  - 합의 버튼을 누른 플레이어가 자동으로 경고 카운터를 초기화하도록 투표 플로우를 확장하고, `autoAdvance`·타임아웃 경로도 공용 경고 루틴을 거치게 했습니다.
  - `turnVoteController`가 `proxy/대역` 상태를 투표 대상에서 제외하도록 보강하고, 엔진에서 `realtimePresence` 스냅샷을 노출해 대시보드·UI에서 재사용할 수 있게 준비했습니다.
- **다음 단계 메모**:
  - 비실시간 매치용 `asyncSessionManager`를 설계해 대역 API 호출 파이프라인과 경고 로깅을 공용 인터페이스로 묶기.
  - 난입 큐(`dropInQueueService`)와 API 키 풀을 서비스 계층으로 구현하고, 경고/대역 이벤트를 서버 로그·관전 UI와 동기화하는 백엔드 훅을 마련하기.
  - 경고/대역 처리 결과를 운영 대시보드와 히스토리 리플레이에서 재구성할 수 있도록 이벤트 포맷과 스냅샷 스키마를 정의하기.

---
느낀 점: 실시간 경고/대역 흐름을 서비스로 분리하니 타이머·투표·난입 보너스가 하나의 상태 머신에서 굴러가 장기적으로 보수하기 쉬워졌습니다.
추가로 필요한 점: 경고와 대역 전환 이벤트를 서버 로그/관전 클라이언트까지 흘려보낼 전송 규격을 조기 정의해야 추후 재생 기능을 붙일 때 혼선이 줄겠습니다.
진행사항: Phase 1·2에 이어 Phase 3의 실시간 관리 축을 착수해 경고 누적·대역 전환·투표 스냅샷 연동까지 기본 골격을 마련했습니다.
