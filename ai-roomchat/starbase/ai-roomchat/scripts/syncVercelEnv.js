...
async function getProjectIdOrThrow({ token, projectId, projectName, teamId }) {
  ...
  const url = withTeamQuery(`https://api.vercel.com/v9/projects/${encodeURIComponent(projectName)}`, teamId)
...
async function setEnvVar({ token, projectId, key, value, targets, teamId }) {
  const url = withTeamQuery(`https://api.vercel.com/v9/projects/${encodeURIComponent(projectId)}/env`, teamId)
  const payload = {
    key,
    value,
    target: targets,
...
async function main() {
  ...
  const resolvedProjectId = await getProjectIdOrThrow({ token, projectId, projectName, teamId })
    ...
    for (const { key, value } of setPairs) {
      const res = await setEnvVar({ token, projectId: resolvedProjectId, key, value, targets, teamId })
