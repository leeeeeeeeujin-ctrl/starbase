...
function buildAlertPayload(event, docUrl, { retryPlan } = {}) {
  ...
  const retryEtaCandidate =
    retryPlan?.recommendedRunAt || retryPlan?.nextRetryEta || event.nextRetryEta || null;
...
async function postJson(
  url,
  body,
  { method = 'POST', headers = {}, timeoutMs = DEFAULT_TIMEOUT_MS } = {}
) {
...
export function mergeCooldownMetadata(existingMetadata, automationSummary) {
  ...
  const alertDocUrl = automationSummary.alertDocUrl || automationSummary.alert?.docUrl || null;
  ...
  const merged = {
    ...
    cooldownAutomation: {
      ...
      docLinkAttachmentCount: alertDocLinkAttached
        ? docLinkAttachmentCount + 1
