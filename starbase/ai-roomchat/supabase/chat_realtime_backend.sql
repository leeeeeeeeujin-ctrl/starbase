...
using (
  ...
  or exists (
    select 1
    from public.chat_room_members moderators
    where moderators.room_id = chat_room_members.room_id
      and moderators.owner_id = auth.uid()
      and moderators.is_moderator is true
  )
)
with check (
  auth.uid() = owner_id
