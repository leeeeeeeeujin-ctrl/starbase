...
import { getRealSimulation } from '@/lib/realTableSimulator';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const data = await getRealSimulation(supabaseAdmin, sessionId);
    return res.status(200).json(data);
  } catch (e) {
    return res.status(500).json({ error: e.message || 'detail_failed' });
  }
}
