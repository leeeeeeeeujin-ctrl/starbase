...
## 구현 진행 현황 업데이트
- **진행률**: 15% – Phase 1의 `turnTimerService`와 `autoAdvanceScheduler` 토대를 구현해 첫 턴 +30초·난입 턴 +40초 보너스, 시간 만료 자동 진행 흐름을 코드에 반영했습니다.
- **완료 사항**:
  - `components/rank/StartClient/services/turnTimerService`를 추가해 기본 제한시간과 보너스 산출을 일원화하고, 세션 시작 시 리셋하도록 연결했습니다.
  - `useStartClientEngine`에서 턴 개시마다 서비스 기반으로 마감 시간을 재계산하고, 참가자 목록 증가 시 난입 보너스를 즉시/다음 턴에 반영하도록 감지했습니다.
  - `StartClient` UI가 타이머가 0초가 되면 합의 상태와 무관하게 자동 진행을 호출해 청사진의 “자동 진행형” 요구사항을 충족합니다.
- **다음 단계 메모**:
  - `turnVoteController`와 서버 로그를 연계해 80% 조기 종료 투표 결과를 세션 기록과 동기화.
  - 난입 큐(`dropInQueueService`)와 API 키 풀 설계를 시작해 프런트 난입 감지 로직과 백엔드 교대 흐름을 잇기.

---
느낀 점: 타이머 서비스를 실제 코드에 얹어 보니 첫 턴/난입 보너스가 명시적으로 보존돼 사용자가 “왜 시간이 늘었는지”를 바로 체감할 수 있을 것 같습니다.
추가로 필요한 점: 실시간 합의(80%)와 타이머 만료 자동 진행 간 우선순위를 정리하고, 서버 이벤트 기록과 동기화하는 인터페이스 정의가 필요합니다.
진행사항: Phase 1 중 타이머·자동 진행 축을 착수해 `StartClient`에서 시간 계산/자동 진행 흐름을 구축하고, 난입 보너스 감지까지 연결했습니다.
