...
function alreadySuppressed(lines, idx) {
  ...
  if (prev.includes('eslint-disable-next-line') && prev.includes('react-hooks/exhaustive-deps')) return true
  // also check two lines above
  if (idx - 2 >= 0) {
    const prev2 = lines[idx - 2].trim()
    if (prev2.includes('eslint-disable-next-line') && prev2.includes('react-hooks/exhaustive-deps')) return true
  }
  return false
}
...
function insertSuppression(filePath, lineNumber) {
  ...
  const comment = "// eslint-disable-next-line react-hooks/exhaustive-deps -- auto-suppressed by codemod"
...
function run() {
  ...
  for (const entry of report) {
    ...
    const lines = Array.from(new Set(messages.map(m => Number(m.line)).filter(Boolean))).sort((a,b)=>a-b)
