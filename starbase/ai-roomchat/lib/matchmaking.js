...
export async function pickOpponents({
  ...
  for (const [role, arr] of byRole.entries()) {
    arr.sort(
      (a, b) => Math.abs((a.score ?? 1000) - myScore) - Math.abs((b.score ?? 1000) - myScore)
    );
  }
  ...
  for (const role of roles) {
    ...
    while (picked.filter(p => p.role === role).length < need && win <= maxWindow) {
      const cand = arr.filter(
        x =>
          Math.abs((x.score ?? 1000) - myScore) <= win && !picked.find(p => p.hero_id === x.hero_id)
      );
      // 부족하면 윈도 확장
      for (const c of cand) {
        picked.push({ role, ...c });
