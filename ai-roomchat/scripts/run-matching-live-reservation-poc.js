...
async function runReservationSim(opts = {}) {
  ...
  for (const g of games)
    metrics.perGame[g.id] = { attempted: 0, success: 0, reservationSucc: 0, reservationFail: 0 };
  ...
  for (let tick = 0; tick < conf.ticks; tick += 1) {
    ...
    for (const g of games) {
      ...
      result.assignments.forEach(a => {
        (a.members || []).forEach(m => {
          if (m && m.id) assignedMemberIds.push(m.id);
        });
      });
      ...
      if (reserveResult && reserveResult.ok) {
        ...
        metrics.success += 1;
        metrics.reservationSucceeded += 1;
        metrics.perGame[g.id].reservationSucc += 1;
      ...
      metrics.perGame[g.id].reservationFail += 1;
      metrics.reservationFailed += 1;
      ...
      const retryResult = matchAsyncParticipants({
        roles: g.roles,
        queue: g.queue.slice(),
        standins: standins.slice(),
        scoreWindows: [100, 200],
      ...
      if (
        retryResult &&
        retryResult.ready === true &&
        Array.isArray(retryResult.assignments) &&
        retryResult.assignments.length > 0
      ) {
        const retryAssignedIds = [];
        retryResult.assignments.forEach(a => {
          (a.members || []).forEach(m => {
            if (m && m.id) retryAssignedIds.push(m.id);
          });
        });
