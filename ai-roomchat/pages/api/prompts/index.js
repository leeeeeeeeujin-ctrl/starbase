import { savePrompt as savePromptInMemory, listPrompts as listPromptsInMemory } from '../../../lib/promptStore'
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      if (supabaseAdmin && supabaseAdmin.from) {
        const { data: inserted, error } = await supabaseAdmin.from('prompts').insert([data]).select().single()
        if (error) throw error
