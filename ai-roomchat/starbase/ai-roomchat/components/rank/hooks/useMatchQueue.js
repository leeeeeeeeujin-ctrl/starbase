...
import { QUEUE_HEARTBEAT_INTERVAL_MS, QUEUE_STALE_THRESHOLD_MS } from '../matchConstants';
import {
  HERO_ID_KEY,
  HERO_OWNER_KEY,
  clearHeroSelection,
...
function normalizeRoleList(list) {
  ...
  return list.map(entry => normalizeRoleEntry(entry)).filter(Boolean);
}
...
async function loadViewerParticipation(
  ...
  try {
    ...
    const resolvedHeroId = guessHeroId || fallbackHeroCandidateId || storedFallbackHeroId || '';
    ...
    if (!Number.isFinite(resolvedScore) || resolvedScore == null) {
      ...
      resolvedScore =
        Number.isFinite(candidateScore) && candidateScore != null ? candidateScore : 1000;
    }
  ...
  const addEntry = (entry, { preferExisting = false } = {}) => {
    ...
    if (indexByHeroId.has(heroId)) {
      ...
      output[index] = {
        ...
        score: Number.isFinite(nextScore)
          ? nextScore
...
export default function useMatchQueue({ gameId, mode, enabled, initialHeroId, onApiKeyExpired }) {
  ...
  useEffect(() => {
    ...
    channel.on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'rank_match_queue', filter },
      handleChange
    );
  ...
  const derived = useMemo(
    () => ({
      ...
      lockedRole,
      roleReady,
      heroMeta: heroId ? heroMap.get(heroId) || heroMap.get(String(heroId)) || null : null,
