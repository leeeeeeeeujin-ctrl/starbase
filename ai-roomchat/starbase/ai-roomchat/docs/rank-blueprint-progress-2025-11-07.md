...
- **오디오 이벤트 관리자 패널**: `/api/admin/audio-events`가 Owner/프로필/히어로/이벤트 유형·검색어 필터와 CSV 내보내기를 처리하고, `trend=weekly` 요청 시 Supabase RPC로 주간 추이를 돌려 관리자 포털 `AudioEventMonitor`의 그래디언트 막대·증감 배지·주간 요약이 자동으로 채워집니다. 동일 테스트 스위트에서 응답·필터 체인·주간 집계 호출·CSV를 검증했습니다.【F:pages/api/admin/audio-events.js†L1-L244】【F:components/admin/AudioEventMonitor.js†L1-L360】【F:styles/AdminPortal.module.css†L203-L389】【F:**tests**/api/admin/audio-events.test.js†L1-L256】
- **오디오 이벤트 Slack 다이제스트**: `scripts/notify-audio-event-trends.js`가 Supabase 집계를 Slack(Webhook)으로 발송하며, 주간/PR CI 워크플로가 실행마다 스크립트를 호출해 알림이 누락되지 않도록 했습니다. 헬퍼 함수는 Jest 단위 테스트로 포맷·요약 로직을 검증합니다.【F:scripts/notify-audio-event-trends.js†L1-L206】【F:.github/workflows/pr-ci.yml†L1-L53】【F:.github/workflows/blueprint-progress-freshness.yml†L1-L53】【F:**tests**/scripts/notify-audio-event-trends.test.js†L1-L66】
- **오디오 이벤트 즐겨찾기 & Slack 구독 저장**: 관리자 포털 `AudioEventMonitor`가 현재 필터를 즐겨찾기로 저장·적용하고, Slack 구독 조건(채널/멘션/임계치/주간 범위/항상 포함)을 폼으로 관리하며 Webhook 키 기반 다중 채널 준비를 지원합니다. `/api/admin/audio-monitor-rules` API와 Supabase `rank_audio_monitor_rules` 테이블이 즐겨찾기/구독 조건을 영속화하고, Slack 다이제스트 스크립트가 구독 조건을 불러와 하이라이트 섹션과 임계치 충족 여부, 급증 배지를 자동으로 추가합니다.【F:components/admin/AudioEventMonitor.js†L340-L820】【F:styles/AdminPortal.module.css†L840-L1110】【F:pages/api/admin/audio-monitor-rules.js†L1-L240】【F:supabase.sql†L320-L380】【F:scripts/notify-audio-event-trends.js†L1-L420】【F:**tests**/api/admin/audio-monitor-rules.test.js†L1-L220】【F:**tests**/scripts/notify-audio-event-trends.test.js†L1-L160】
- **오디오 이벤트 즐겨찾기 & Slack 구독 저장**: 관리자 포털 `AudioEventMonitor`가 현재 필터를 즐겨찾기로 저장·적용하고, Slack 구독 조건(채널/멘션/임계치/주간 범위/항상 포함)을 폼으로 관리하며 Webhook 키 기반 다중 채널 준비를 지원합니다. `/api/admin/audio-monitor-rules` API와 Supabase `rank_audio_monitor_rules` 테이블이 즐겨찾기/구독 조건을 영속화하고, Slack 다이제스트 스크립트가 구독 조건을 불러와 하이라이트 섹션과 임계치 충족 여부, 급증 배지를 자동으로 추가합니다.【F:components/admin/AudioEventMonitor.js†L340-L820】【F:styles/AdminPortal.module.css†L840-L1110】【F:pages/api/admin/audio-monitor-rules.js†L1-L240】【F:supabase.sql†L320-L380】【F:scripts/notify-audio-event-trends.js†L1-L420】【F:**tests**/api/admin/audio-monitor-rules.test.js†L1-L220】【F:**tests**/scripts/notify-audio-event-trends.test.js†L1-L160】
- **저장 규칙 자동 복원**: `AudioEventMonitor`가 마지막으로 적용한 즐겨찾기·Slack 구독 조건을 `localStorage`에 보존하고, 관리자 포털 재방문 시 자동 복원해 필터·그래프·Slack 구독 요약이 즉시 동일 상태로 열립니다. 패널 상단의 활성 규칙 배지가 적용 시간과 저장 시각을 동시에 표시해 오래된 조건을 바로 파악할 수 있습니다.【F:components/admin/AudioEventMonitor.js†L620-L920】【F:styles/AdminPortal.module.css†L1030-L1100】
...
- **단계 진행도 갱신**: 오디오 연동과 관리자 모니터 즐겨찾기·Slack 구독 저장 흐름이 마무리돼 UI·오디오 단계가 100%에 도달했고, 전체 청사진 진행도 추산도 약 76%로 갱신했습니다.【F:docs/rank-blueprint-overview.md†L18-L70】
