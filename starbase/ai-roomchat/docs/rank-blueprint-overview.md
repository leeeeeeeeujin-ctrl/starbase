...
- [오디오 이벤트 모니터링 패널](components/admin/AudioEventMonitor.js): 관리자 포털에 오디오 변경 로그 패널을 추가해 운영자가 Owner/프로필/히어로/이벤트 유형·검색어 필터로 추려보고, 최근 24시간/7일/30일 스냅샷을 전환하거나 현재 뷰를 CSV로 내려받을 수 있습니다. 동일한 필터 세트를 `/api/admin/audio-events`에서 처리하고, 테스트 스위트(`__tests__/api/admin/audio-events.test.js`)로 CSV 응답·필터 체인을 커버했습니다.【F:components/admin/AudioEventMonitor.js†L1-L260】【F:styles/AdminPortal.module.css†L203-L516】【F:pages/api/admin/audio-events.js†L1-L214】【F:__tests__/api/admin/audio-events.test.js†L1-L213】
...
| 2 | 오디오 이벤트 로그의 주간 추이·담당자별 요약을 그래프로 시각화하고 Slack 알림에 연동하는 Telemetry 대시보드 확장을 설계합니다. | 프론트 (라라) | 2025-12-02 | P1 (오디오 튜닝) | 프론트 3일 (1명·UI/데이터) |
...
| UI·오디오 완성 | 진행 중 | 92% | 메인 룸 히어로 패널이 브금 자동 재생·컨트롤을 제공하고, StartClient 로그 보드는 섹션별 축약/펼치기 토글·검색 입력에 더해 검색어 하이라이트와 액션/주역/태그 다중 필터 칩을 갖춰 긴 히스토리에서도 필요한 카드만 빠르게 추리며, 모바일에선 히스토리 패널이 세로로 재배치되고 데스크톱에선 다단 그리드로 확장돼 전투/AI/플레이어 히스토리를 화면 크기에 맞춰 보여줍니다. 브금 재생목록 전환, EQ/리버브/컴프레서 토글, 프리셋 칩까지 연동된 상태에서 선택한 트랙과 이펙트는 사용자·운영자별로 저장되고 Supabase 로그로 남아 재진입 시 자동 복원됩니다. 관리자 포털에는 Owner/히어로/이벤트 유형·검색어 필터와 기간 스위치를 갖춘 오디오 이벤트 패널이 추가돼, 동일 API를 통해 CSV를 내려받으며 최근 변경 로그를 바로 검토할 수 있습니다. |
