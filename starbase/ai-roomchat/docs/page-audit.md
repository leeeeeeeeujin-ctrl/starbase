...
- Uses `Home.module.css` for glassmorphism styling, a core feature list, and blueprint overview widgets that read JSON-backed stage progress and “다음 액션” snapshots with a freshness badge so visitors can see both milestone status and the remaining checklist. `npm run refresh:blueprint-progress` regenerates the JSON payloads from the overview doc so the UI stays aligned without manual edits, and `npm run check:blueprint-progress-freshness` (also executed weekly via GitHub Actions) fails when the snapshot is older than the 14일 한계치 so maintainers receive a reminder.【F:pages/index.js†L52-L142】【F:styles/Home.module.css†L1-L212】【F:data/rankBlueprintProgress.json†L1-L35】【F:data/rankBlueprintNextActions.json†L1-L21】【F:scripts/refresh-rank-blueprint-progress.js†L1-L214】【F:scripts/check-rank-blueprint-progress-freshness.js†L1-L74】【F:.github/workflows/blueprint-progress-freshness.yml†L1-L20】
