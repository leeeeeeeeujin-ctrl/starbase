...
export async function createRealSimulation(supabase, options) {
  ...
  console.log(`[realTableSimulator] Total slots needed: ${totalSlots}, Heroes provided: ${heroes.length}`)
  ...
  console.log('[realTableSimulator] Roles for matching:', JSON.stringify(
    roles.map(r => ({ name: r.name, slotCount: r.slot_count })), null, 2
  ))
  ...
  // 디버그: 매칭 결과 출력
  console.log('[realTableSimulator] Match result:', JSON.stringify({
...
export async function getRealSimulation(supabase, sessionId) {
  const { data: session, error: sessionError } = await supabase
    ...
    .eq('id', sessionId)
    .single()
  if (sessionError || !session) throw new Error(`세션을 찾을 수 없습니다: ${sessionError?.message}`)
  ...
  const battleIds = Array.from(new Set((logs || [])
    .filter(l => (l.meta && (l.meta.session_id === sessionId)))
    .map(l => l.battle_id)))
...
export async function advanceRealSimulationTurn(supabase, sessionId, aiClient = null) {
  ...
  if (!Array.isArray(slots) || slots.length < 2) throw new Error('배틀을 진행할 슬롯이 부족합니다.')
