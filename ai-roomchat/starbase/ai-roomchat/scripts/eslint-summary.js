...
fs.writeFileSync(OUT, JSON.stringify({ generatedAt: new Date().toISOString(), summary }, null, 2), 'utf8')

console.log('Wrote summary to', OUT)
console.log('Top rules:')
console.log(summary.topRules.slice(0, 20).map(r => `${r.key}: ${r.count}`).join('\n'))
console.log('\nTop files:')
console.log(summary.topFiles.slice(0, 20).map(f => `${f.key}: ${f.count}`).join('\n'))
