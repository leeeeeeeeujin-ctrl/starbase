...
function normalizeGroup(group = {}) {
  return {
    warning:
      typeof group.warning === 'number' || group.warning === null ? group.warning : undefined,
    critical:
      typeof group.critical === 'number' || group.critical === null ? group.critical : undefined,
  };
}
...
function buildDiff(previous = {}, next = {}) {
  ...
  for (const key of keys) {
    ...
    if (!Object.is(prevGroup.warning, nextGroup.warning)) {
      changes.warning = {
        before: prevGroup.warning === undefined ? null : (prevGroup.warning ?? null),
        after: nextGroup.warning === undefined ? null : (nextGroup.warning ?? null),
      };
    }

    if (!Object.is(prevGroup.critical, nextGroup.critical)) {
      changes.critical = {
        before: prevGroup.critical === undefined ? null : (prevGroup.critical ?? null),
        after: nextGroup.critical === undefined ? null : (nextGroup.critical ?? null),
      };
    }
...
function buildTimeline(
  ...
  if (normalizedMode === 'weekly') {
    const normalizedWeeks =
      Number.isFinite(windowWeeks) && windowWeeks > 0
  ...
  if (normalizedMode === 'monthly') {
    const normalizedMonths =
      Number.isFinite(windowMonths) && windowMonths > 0
  ...
  const normalizedDays =
    Number.isFinite(windowDays) && windowDays > 0 ? Math.min(Math.floor(windowDays), limit) : 14;
  if (!normalizedDays) {
...
async function publishSlackNotification(event) {
  ...
  try {
    await postJson(
      ...
      {
        headers: ALERT_AUDIT_AUTH_HEADER ? { Authorization: ALERT_AUDIT_AUTH_HEADER } : {},
      }
    );
...
export function summarizeCooldownThresholdAuditTrail(
  ...
  const normalizedWindow =
    Number.isFinite(windowMs) && windowMs > 0 ? windowMs : 48 * 60 * 60 * 1000;
...
export function recordCooldownThresholdChange({ previous, next, context = {} }) {
  if (!next) {
