...
function normaliseRuleFilters(raw = {}) {
  const eventTypes = Array.isArray(raw.eventTypes)
    ? Array.from(new Set(raw.eventTypes.map(value => sanitiseString(value, 48)).filter(Boolean)))
    : [];
...
function normaliseSlackConfig(raw = {}) {
  ...
  const minEvents =
    Number.isFinite(minEventsValue) && minEventsValue > 0 ? Math.min(minEventsValue, 500) : 1;
  const lookbackWeeks =
...
function normaliseBuckets(raw = [], { now = new Date(), weeks = DEFAULT_LOOKBACK_WEEKS } = {}) {
  const rangeWeeks =
    Number.isFinite(weeks) && weeks > 0
      ? Math.min(Math.floor(weeks), MAX_LOOKBACK_WEEKS)
      : DEFAULT_LOOKBACK_WEEKS;
...
async function fetchBreakdown(client, options = {}, mode = 'hero') {
  return client.rpc('rank_audio_events_weekly_breakdown', {
    ...
    event_type_filter:
      Array.isArray(options.eventTypes) && options.eventTypes.length ? options.eventTypes : null,
  });
}
...
function detectAnomaly(summary) {
  ...
  if (
    direction === 'up' &&
    (magnitude >= ANOMALY_PERCENT_THRESHOLD || absoluteDelta >= ANOMALY_MIN_ABSOLUTE_DELTA)
  ) {
    return {
      type: 'spike',
      badge: '🔺 급증 감지',
  ...
  for (const rule of subscriptions) {
    try {
      ...
      const lookbackWeeks = Math.min(
        Math.max(slack.lookbackWeeks || DEFAULT_LOOKBACK_WEEKS, 1),
        MAX_LOOKBACK_WEEKS
      );
      ...
      let query = client
        ...
        .select('id, hero_name, hero_source, profile_key, event_type, details, created_at', {
          count: 'exact',
        })
      ...
      if (trendResponse.error) {
        console.warn(
          '[audio-events] Failed to fetch subscription trend',
          rule.id,
          trendResponse.error
        );
      }
      ...
      console.warn(
        '[audio-events] Unexpected error building subscription highlight',
        rule?.id,
        error
      );
    }
...
async function main() {
  ...
  const lookbackWeeks =
    Number.isFinite(lookbackInput) && lookbackInput > 0
  ...
  if (!buckets.some(bucket => bucket.eventCount > 0)) {
    console.log(
      '[audio-events] No events recorded in lookback window, skipping Slack notification.'
    );
    return;
  }
  ...
  if (breakdownResponse.error) {
    console.warn(
      '[audio-events] Failed to fetch hero distribution for Slack payload',
  ...
  const subscriptionHighlights = await buildSubscriptionHighlights(
    client,
