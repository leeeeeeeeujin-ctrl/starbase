...
async function fetchParticipantsWithHeroes(gameId) {
  ...
  const { data: heroesData, error: heroError } = await supabase
    .from('heroes')
    .select(
...
export function useGameRoom(
  ...
  const deleteRoom = useCallback(async () => {
    ...
    try {
      ...
      await supabase.from('rank_participants').delete().eq('game_id', gameId)
      await supabase.from('rank_game_slots').delete().eq('game_id', gameId)
      await supabase.from('rank_games').delete().eq('id', gameId)
      onDeleted?.()
      return { ok: true }
    } catch (err) {
