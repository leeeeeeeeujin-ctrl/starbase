...
function normalizeEntry(entry) {
  ...
  return {
    ...
    type: mimeType,
    displayType: type,
    name: entry.name || entry.filename || `${id}.${type === 'image' ? 'jpg' : type === 'video' ? 'mp4' : 'bin'}`,
...
function normalizeTimelineResponse(response) {
  const items = Array.isArray(response?.entries) ? response.entries : Array.isArray(response) ? response : []
  const entries = items
...
export async function fetchNativeMediaAsset({
  ...
  if (!blob && response.base64) {
    blob = base64ToBlob(response.base64, response.mimeType || response.type || 'application/octet-stream')
  }
  ...
  return {
    blob,
    name: response.name || response.filename || `${id}.${mediaType === 'video' ? 'mp4' : mediaType === 'image' ? 'jpg' : 'bin'}`,
