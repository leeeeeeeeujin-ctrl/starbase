...
async function runSelftest() {
  ...
  try {
    ...
    pushStep('fetch-data', 'ok', { gameId: game.id, gameName: game.name, heroCount: heroes.length })

    // 2) 생성
    const pickIds = heroes.slice(0, Math.min(4, heroes.length)).map(h => h.id)
    console.log(`\n📝 Creating simulation with heroes: ${heroes.slice(0, 4).map(h => h.name).join(', ')}`)
    ...
    results.forEach((turn, i) => {
      console.log(`   Turn ${i + 1}: ${turn.attackerName} → ${turn.defenderName} (damage: ${turn.damage})`)
    })
