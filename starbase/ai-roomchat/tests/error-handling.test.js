...
class ErrorHandlingValidator {
  ...
  testUserInputHandling() {
    ...
    const generateCharacterVariables = character => {
      try {
        ...
        return {
          '{{캐릭터.이름}}':
            character.name != null && character.name !== '' ? String(character.name) : '익명',
  ...
  async runAllErrorTests() {
    ...
    console.log(
      `   API 에러 처리: ${apiResults.filter(r => r.passed).length}/${apiResults.length} 통과`
    );
    console.log(
      `   입력 에러 처리: ${inputResults.filter(r => r.passed && r.errorHandled).length}/${inputResults.length} 통과`
    );
    console.log(
      `   시스템 에러 처리: ${systemResults.filter(r => r.passed && r.errorHandled).length}/${systemResults.length} 통과`
    );
    console.log(
      `   총 통과율: ${Math.round((passedTests / totalTests) * 100)}% (${passedTests}/${totalTests})`
    );
