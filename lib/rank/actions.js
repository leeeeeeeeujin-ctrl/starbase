...
// - All paths are first resolved relative to BASE_ROOT (see resolveSafe).
...
async function action_list_files(payload){ const dir = ensureReadableRoot(resolveSafe(payload?.path||'.')); const recursive=!!payload?.recursive; const items = await listDir(dir, recursive); return { ok:true, result:{ items } }; }
async function action_read_file(payload){ const file=ensureReadableFile(resolveSafe(payload?.path)); const stat=await fs.lstat(file); if (stat.size>MAX_FILE_BYTES) return { ok:false, error:'file_too_large' }; const buf=await fs.readFile(file); if (isBinaryBuffer(buf)) return { ok:true, result:{ encoding:'base64', content: buf.toString('base64') } }; return { ok:true, result:{ encoding:'utf8', content: buf.toString('utf8') } }; }
async function action_read_file_range(payload){ const file=ensureReadableFile(resolveSafe(payload?.path)); const start=Number(payload?.start??0); const end=Number(payload?.end??start+250); const txt=await fs.readFile(file,'utf8'); const lines=txt.split(/\r?\n/); const s=Math.max(0,start); const e=Math.min(lines.length, Math.max(s,end)); const slice=lines.slice(s,e).join('\n'); return { ok:true, result:{ start:s, end:e, content:slice } }; }
async function action_write_file(payload){ const file=ensureWritablePath(resolveSafe(payload?.path)); const content=typeof payload?.content==='string'?payload.content:''; await fs.mkdir(path.dirname(file),{recursive:true}); await fs.writeFile(file, content, 'utf8'); return { ok:true }; }
async function action_delete_file(payload){ const file=ensureWritablePath(resolveSafe(payload?.path)); await fs.rm(file,{force:true}); return { ok:true }; }
async function action_delete_dir(payload){ const dir=ensureWritablePath(resolveSafe(payload?.path)); const recursive=payload?.recursive!==false; await fs.rm(dir,{recursive,force:true}); return { ok:true }; }
async function action_move_file(payload){ const src=ensureWritablePath(resolveSafe(payload?.src)); const dest=ensureWritablePath(resolveSafe(payload?.dest)); await fs.mkdir(path.dirname(dest),{recursive:true}); await fs.rename(src,dest); return { ok:true }; }
async function action_copy_file(payload){ const src=ensureWritablePath(resolveSafe(payload?.src)); const dest=ensureWritablePath(resolveSafe(payload?.dest)); await fs.mkdir(path.dirname(dest),{recursive:true}); await fs.copyFile(src,dest); return { ok:true }; }
async function action_mkdirs(payload){ const dir=ensureWritablePath(resolveSafe(payload?.path)); await fs.mkdir(dir,{recursive:true}); return { ok:true }; }
async function action_stat_file(payload){ const p=ensureReadableFile(resolveSafe(payload?.path)); const s=await fs.lstat(p); return { ok:true, result:{ isDir:s.isDirectory(), size:s.size, mtimeMs:s.mtimeMs } }; }
async function action_search_text(payload){ const root=ensureReadableRoot(resolveSafe(payload?.path||'.')); const query=String(payload?.query||'').trim(); if(!query) return { ok:false, error:'missing_query' }; const maxResults=Number(payload?.max_results||SEARCH_MAX_RESULTS_DEFAULT); const results=[]; async function scan(d){ const entries=await fs.readdir(d,{withFileTypes:true}); for(const ent of entries){ if(results.length>=maxResults) return; if(ent.name==='.git'||ent.name==='node_modules') continue; const full=path.join(d,ent.name); if(ent.isDirectory()){ await scan(full);} else { try{ const stat=await fs.lstat(full); if(stat.size>MAX_FILE_BYTES) continue; const buf=await fs.readFile(full); if(isBinaryBuffer(buf)) continue; const text=buf.toString('utf8'); const lines=text.split(/\r?\n/); for(let i=0;i<lines.length;i++){ if(lines[i].includes(query)){ results.push({ path:path.relative(BASE_ROOT, full), line:i+1, preview:lines[i] }); if(results.length>=maxResults) break; } } }catch{} } } } await scan(root); return { ok:true, result:{ results } }; }
async function action_edit_patch(payload){ const diff=String(payload?.diff||payload?.patch||'').trim(); const cwd=BASE_ROOT; if(!diff) return { ok:false, error:'missing_patch' }; // Ensure all touched paths are inside the workspace subtree
  const lines = diff.split(/\r?\n/); for (const line of lines){ if (!(line.startsWith('+++ ') || line.startsWith('--- '))) continue; const parts=line.split(/\s+/); if(parts.length<2) continue; let p=parts[1]; if(p==='/dev/null') continue; if(p.startsWith('a/')) p=p.slice(2); else if(p.startsWith('b/')) p=p.slice(2); if(p.startsWith('\"')&&p.endsWith('\"')) p=p.slice(1,-1); if(!p) continue; let abs; try{ abs=resolveSafe(p);}catch{ return { ok:false, error:'patch_paths_not_allowed' }; } try{ ensureWritablePath(abs);}catch{ return { ok:false, error:'patch_paths_not_allowed' }; } } const tmp=await fs.mkdtemp(path.join(os.tmpdir(),'patch-')); const patchPath=path.join(tmp,'patch.diff'); await fs.writeFile(patchPath,diff,'utf8'); const cmd=`git apply --unsafe-paths --reject --whitespace=nowarn "${patchPath.replace(/\"/g,'\\\"')}"`; const r=await runCommand(cmd,{cwd,timeoutMs:20000}); try{ await fs.rm(tmp,{recursive:true,force:true}); }catch{} if(!r.ok) return { ok:false, error:'patch_failed', detail:{ code:r.code, stdout:r.stdout, stderr:r.stderr } }; return { ok:true }; }
...
async function action_ui_sandbox_step(payload){
  ...
  if(!baseUrl) return { ok:false, error:'ui_sandbox_not_configured' };
  const fnFetch = typeof fetch==='function'
    ? fetch
    : (typeof globalThis!=='undefined' && typeof globalThis.fetch==='function' ? globalThis.fetch : null);
  if(!fnFetch) return { ok:false, error:'ui_sandbox_fetch_unavailable' };
  ...
  const sessionOptions = payload && typeof payload.sessionOptions==='object' && payload.sessionOptions!==null ? payload.sessionOptions : {};
  ...
  try{
    ...
    if(!sessionId){
      const resCreate = await fnFetch(`${base}/session`, { method:'POST', headers, body: JSON.stringify(sessionOptions) });
      const dataCreate = await resCreate.json().catch(()=>null);
      if(!resCreate.ok || !dataCreate || !dataCreate.sessionId){
        return { ok:false, error:'ui_sandbox_session_failed', detail:dataCreate||null };
      }
      sessionId = String(dataCreate.sessionId);
    }
    const resStep = await fnFetch(`${base}/session/${encodeURIComponent(sessionId)}/step`, {
      method:'POST',
      headers,
    ...
    if(!resStep.ok || !dataStep){
      return { ok:false, error:'ui_sandbox_step_failed', detail:dataStep||null };
    }
    const state = (dataStep.state && typeof dataStep.state==='object') ? dataStep.state : (dataStep.result || dataStep);
    return { ok:true, result:{ sessionId, state } };
  }catch(err){
    return { ok:false, error:'ui_sandbox_error', detail:String(err?.message||err) };
  }
}

const registry = { list_files:action_list_files, read_file:action_read_file, read_file_range:action_read_file_range, write_file:action_write_file, delete_file:action_delete_file, delete_dir:action_delete_dir, move_file:action_move_file, copy_file:action_copy_file, mkdirs:action_mkdirs, stat_file:action_stat_file, search_text:action_search_text, edit_patch:action_edit_patch, sandbox_exec:action_sandbox_exec, test_run:action_test_run, lint_run:action_lint_run, build_run:action_build_run, ui_sandbox_step: action_ui_sandbox_step };
