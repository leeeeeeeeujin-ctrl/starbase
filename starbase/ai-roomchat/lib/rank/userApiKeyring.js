...
export async function insertUserApiKeyringEntry({
  ...
  const normalizedProvider = typeof provider === 'string' ? provider.trim().toLowerCase() : 'unknown'
...
export async function deleteUserApiKeyringEntry({ userId, entryId }) {
  ...
  const { error } = await supabaseAdmin.from(TABLE).delete().eq('id', entryId).eq('user_id', userId)
