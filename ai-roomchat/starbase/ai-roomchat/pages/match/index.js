...
const styles = {
  ...
  gameCard: selected => ({
    borderRadius: 18,
    border: selected ? '1px solid rgba(59, 130, 246, 0.6)' : '1px solid rgba(148, 163, 184, 0.28)',
  ...
  keyringRow: active => ({
    ...
    padding: '10px 14px',
    borderRadius: 16,
    border: active ? '1px solid rgba(56, 189, 248, 0.5)' : '1px solid rgba(148, 163, 184, 0.24)',
...
function normalizeLobbySnapshot(raw = {}) {
  const queue = Array.isArray(raw.queue) ? raw.queue.map(normalizeQueueTicket).filter(Boolean) : [];
  ...
  return {
    ...
    heroScoreRange:
      row.hero_score_min !== null || row.hero_score_max !== null
        ...
        : null,
    heroRoleDeltaMin: Number.isFinite(row.hero_role_delta_min)
      ? Number(row.hero_role_delta_min)
      : null,
    heroRoleDeltaMax: Number.isFinite(row.hero_role_delta_max)
...
export default function MatchPage() {
  ...
  const hasActiveKey = useMemo(
    () => keyringEntries.some(entry => entry?.isActive),
    [keyringEntries]
  );
        ...
        if (profile?.hero_id) {
          try {
            ...
            if (!cancelled && mountedRef.current) {
              const participations = Array.isArray(bundle?.participations)
  ...
  useEffect(() => {
    ...
    if (ticket.status === 'staging') {
      const deadline = ticket.readyExpiresAt
        ? Date.parse(ticket.readyExpiresAt)
        : Date.now() + MATCH_READY_DURATION_MS;
  ...
  const handleStartMatch = useCallback(async () => {
    ...
    if (penaltyUntil && Date.now() < penaltyUntil) {
      setQueueError(
        new Error('최근 취소로 잠시 매칭을 이용할 수 없습니다. 잠시 후 다시 시도해 주세요.')
      );
      return;
    }
    setQueueError(null);
  ...
  useEffect(() => {
    if (!ticket?.id) return;
    const snapshotSession = snapshot.sessions.find(
      entry => entry.roomId && entry.roomId === ticket.roomId
    );
    if (snapshotSession) {
    ...
    <section style={styles.section}>
      <div style={styles.sectionHeader}>
        ...
        <p style={styles.sectionHint}>
          AI 모델 키를 등록하고 활성화해야 매칭을 시작할 수 있습니다.
        </p>
      </div>
      ...
      <div style={styles.keyringList}>
        ...
        {keyringEntries.length === 0 && !keyringLoading ? (
          <p style={styles.sectionHint}>
            등록된 API 키가 없습니다. 위 입력창에 키를 붙여넣고 저장해 주세요.
          </p>
        ) : null}
      </div>
    </section>
    ...
    <section style={styles.section}>
      ...
      {viewerHero ? (
        <div style={styles.heroBadge}>
          ...
          {viewerHero.hero_id ? shortId(viewerHero.hero_id) : 'Hero'}
        </div>
      ) : (
        ...
        {games.map(game => (
          ...
          >
            ...
            <p style={styles.gameDescription}>
              {game.description || '설명이 제공되지 않은 게임입니다.'}
            </p>
  ...
  const renderMatchSection = () => (
    <section style={styles.section}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        <button type="button" style={styles.smallButton(cancelBusy)} onClick={handleCancelMatch}>
          {cancelBusy ? '취소 중…' : '매칭 취소'}
        </button>
      </div>
      {penaltyUntil && Date.now() < penaltyUntil ? (
        <p style={styles.errorText}>
          최근 취소로 인해 {Math.ceil((penaltyUntil - Date.now()) / 1000)}초 후 다시 참여할 수
          있습니다.
        </p>
      ) : null}
      {queueError ? (
        <p style={styles.errorText}>{queueError.message || '매칭 작업 중 오류가 발생했습니다.'}</p>
      ) : null}
      <div style={styles.statusCard}>
        ...
        {activeMatchSummary ? (
          <>
            <p style={styles.statusHint}>
              티켓 {shortId(activeMatchSummary.ticketId)} ·{' '}
              {activeMatchSummary.status.toUpperCase()}{' '}
            ...
            {matchReadyState.active ? (
              <div>
                ...
                <p style={styles.sectionHint}>
                  남은 시간 {Math.max(0, Math.ceil((matchReadyState.deadline - Date.now()) / 1000))}
                  초
                </p>
                {timeSelectionError ? (
                  <p style={styles.errorText}>
                    {timeSelectionError.message || '턴 제한 적용에 실패했습니다.'}
                  </p>
                ) : null}
              </div>
            ) : null}
        ...
        {snapshotError ? (
          <p style={styles.errorText}>
            {snapshotError.message || '로비 스냅샷을 불러오지 못했습니다.'}
          </p>
        ) : null}
      </div>
    </section>
        ...
        <div style={styles.container}>
          <header style={styles.header}>
            ...
            <p style={styles.subtitle}>
              API 키를 활성화하고 게임을 선택한 뒤 매칭 버튼만 누르면 자동으로 큐·방·메인 게임까지
              연결됩니다.
            </p>
