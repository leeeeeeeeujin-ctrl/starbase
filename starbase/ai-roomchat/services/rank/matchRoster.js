...
function normalizeEntry(raw, index) {
  ...
  const ownerId = toUuid(
    raw.ownerId ?? raw.owner_id ?? raw.occupantOwnerId ?? raw.ownerID,
  )
  ...
  const heroSummary = toPlainObject(
    raw.heroSummary ?? raw.hero_summary ?? raw.heroMeta ?? {},
  )
  ...
  const status =
    toTrimmed(raw.status ?? raw.standinStatus) || (standin ? 'standin' : null)
...
export function hydrateRoster({
  ...
  return roster.map((entry) => {
    ...
    const heroMeta =
      heroSummaryMap.get(entry.hero_id) || heroMap[entry.hero_id || ''] || null

    const standinFlag = ownerStats.standin === true || entry.standin === true
    const matchSource =
      entry.match_source ||
      ownerStats.match_source ||
      (standinFlag ? 'async_standin' : null)
