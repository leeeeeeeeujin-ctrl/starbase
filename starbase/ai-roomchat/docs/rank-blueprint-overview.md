...
> _2025-11-16 기준 자동 생성된 기한 알림._
> 📅 #1 (프론트 (라라)) 마감이 3일 남았습니다.
> 📅 #4 (Matchmaking (류·라라)) 마감이 일주일 남았습니다.
...
| 4 | 샘플 메타 기반 재시도/경고 UX를 QA하고, 솔로·듀오·캐주얼 비실시간 매칭 체크리스트를 작성합니다. | Matchmaking (류·라라) | 2025-11-24 | P1 (매칭 안정화) | 프론트·백엔드 3일 (UX/로직 QA) |

<!-- blueprint-next-actions:end -->

<!-- blueprint-progress:start -->
## 7. 진행률 현황 (2025-11-16 기준)
...
| 실시간·난입 매칭 계층화 | 구현 진행 중 | 65% | 실시간 난입 파이프라인에 이어 비실시간 샘플링·표본 메타 리포트를 통합해 표준 매칭 응답 구조를 정비했습니다. 남은 QA/재시도 UX를 진행 중입니다. |

**총 진행률(단계별 동일 가중치)**: 약 **88%**

<!-- blueprint-progress:end -->

<!-- blueprint-remaining:start -->
### 8. 남은 청사진 핵심 작업 (2025-11-16 기준)
| 단계 | 남은 핵심 작업 | 현재 진행률 |
| --- | --- | --- |
...
| 실시간·난입 매칭 계층화 | 샘플 메타 기반 재시도 UX와 비실시간 QA 체크리스트를 확정합니다. | 65% |
