...
export function useRankingShowcaseData({ maxGameSections = 3 } = {}) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error: listError } = await withTable(supabase, 'rank_participants', table =>
        supabase
          .from(table)
      ...
      if (gameRes.error)
        setError(prev => prev || gameRes.error.message || '게임 정보를 불러오지 못했습니다.');
  ...
  const enrichedRows = useMemo(
    () => rows.map(row => summarize(row, heroMap, gameMap)),
    [rows, heroMap, gameMap]
  );
