...
export default function ChatOverlay({ open, onClose, onUnreadChange }) {
  ...
  const activeRoomId = context?.type === 'room' ? context.roomId : null
    ...
    const load = async () => {
      ...
      try {
        const result = await fetchRecentMessages({
          ...
          chatRoomId: context.roomId || null,
  ...
  const handleLeaveRoom = useCallback(
    async (room) => {
      ...
      try {
        ...
        await refreshRooms()
        if (context?.type === 'room' && context.roomId === room.id) {
          setContext(null)
          setMessages([])
        }
      } catch (error) {
  ...
  const renderRoomList = (visibility) => {
    const list = visibility === 'open' ? rooms.available || [] : rooms.joined || []
    if (!list.length) {
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'grid', gap: 4 }}>
            ...
            <span style={{ fontSize: 12, color: '#94a3b8' }}>
              {context.type === 'session' ? '세션 채팅' : context.visibility === 'open' ? '공개 채팅방' : '비공개 채팅방'}
            </span>
          </div>
