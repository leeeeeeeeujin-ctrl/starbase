...
2. `rank_turns` 가시성/요약 컬럼 초안을 `supabase-ddl-export.md`에 반영하고, 마이그레이션 영향 분석 노트를 추가합니다.
...
| 세션/전투 동기화 | 설계 검토 중 | 45% | `rank_turns` 가시성·요약 컬럼 DDL과 인덱스 초안을 작성해 마이그레이션 리허설을 준비했습니다. |
...
| 운영 가드 | 진행 중 | 45% | Webhook 3-5-10분 백오프 전략에 이어 `rank_api_key_audit` 감사 테이블까지 정의해 자동화/회수 로그 저장 기반을 마련했습니다. |

**총 진행률(단계별 동일 가중치)**: 약 **51%**
