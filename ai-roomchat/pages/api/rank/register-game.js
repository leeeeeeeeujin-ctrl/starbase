...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sanitizedGame = {
      ...
      description: typeof prepared.game.description === 'string' ? prepared.game.description : '',
      realtime_match: realtimeMode,
    };

    const { data: promptSetRows, error: promptSetError } = await withTableQuery(
      supabaseAdmin,
      'prompt_sets',
      from => from.select('id').eq('id', sanitizedGame.prompt_set_id).limit(1)
    );
    ...
    if (verifyError) {
      ...
      if (!missingVerify) {
        console.warn(
          'verify_rank_roles_and_slots RPC failed:',
    ...
    const missingRpc =
      /register_rank_game/i.test(rpcError.message || '') &&
    ...
    if (prepared.slots.length > 0) {
      ...
      const { error: slotError } = await supabaseAdmin.from('rank_game_slots').insert(slotRows);
