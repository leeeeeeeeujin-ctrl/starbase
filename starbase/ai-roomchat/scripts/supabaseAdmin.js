...
async function listSimulations() {
  ...
  const simulations = data.filter(s => 
    s.rank_session_meta?.extras?.simulation === true
  )
...
async function createTestGame(name) {
  ...
  // 기본 역할 생성
  const roles = [
    { game_id: game.id, name: '공격', slot_count: 1, active: true, score_delta_min: 20, score_delta_max: 40 },
    { game_id: game.id, name: '수비', slot_count: 2, active: true, score_delta_min: 20, score_delta_max: 40 },
  ]
