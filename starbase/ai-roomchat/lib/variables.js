...
export function compileVariables({
  ...
  activeGlobal.forEach(v => lines.push(buildActiveVarLine({...v, scope:'전역'}, activeGlobalNames)))
  activeLocal.forEach(v => lines.push(buildActiveVarLine({...v, scope:'로컬'}, activeLocalNames)))
  return lines.join('\n')
}
