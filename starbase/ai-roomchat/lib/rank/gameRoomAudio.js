...
export const DEFAULT_COMPRESSOR_SETTINGS = { enabled: false, threshold: -28, ratio: 2.5, release: 0.25 }
...
export function normalizeHeroAudioProfile(rawHero, { fallbackHeroId = null, fallbackHeroName = '' } = {}) {
  ...
  trackSources.forEach((entry, index) => {
    ...
    const trackId =
      entry.id || entry.key || entry.slug || (typeof entry.label === 'string' && entry.label.trim()) || `track-${index}`
