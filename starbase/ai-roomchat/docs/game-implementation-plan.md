...
- **세션 메타 API 확장**: `/api/rank/session-meta`(신규)에서 세션 토큰을 검사한 뒤 `enqueue_rank_turn_state_event`를 호출하고, `upsert_match_session_meta` 결과를 반환한다. MatchReady와 StartClient는 제한시간 투표·드롭인 보너스·턴 진행 시점마다 이 API를 사용해 서버와 동기화한다.【F:components/rank/MatchReadyClient.js†L250-L312】【F:components/rank/StartClient/useStartClientEngine.js†L900-L1100】
