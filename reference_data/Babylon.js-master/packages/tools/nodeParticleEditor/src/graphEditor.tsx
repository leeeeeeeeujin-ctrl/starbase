...
export class GraphEditor extends React.Component<IGraphEditorProps, IGraphEditorState> {
    ...
    appendBlock(dataToAppend: NodeParticleBlock | INodeData, recursion = true) {
    ...
    addValueNode(type: string) {
        const nodeType: NodeParticleBlockConnectionPointTypes = BlockTools.GetConnectionNodeTypeFromString(type);

        const newInputBlock = new ParticleInputBlock(type, nodeType);
        return this.appendBlock(newInputBlock);
    }

    prepareHistoryStack() {
        const particleSet = this.props.globalState.nodeParticleSet;
        const globalState = this.props.globalState;

        const dataProvider = () => {
            SerializationTools.UpdateLocations(particleSet, globalState);
            return particleSet.serialize();
        };

        const applyUpdate = (data: any) => {
            globalState.stateManager.onSelectionChangedObservable.notifyObservers(null);
            particleSet.parseSerializedObject(data);
    ...
    constructor(props: IGraphEditorProps) {
        ...
        this.props.globalState.onImportFrameObservable.add((source: any) => {
            ...
            const blocks = this.props.globalState.nodeParticleSet.attachedBlocks.slice(-frameData.blocks.length);

            for (const block of blocks) {
                this.appendBlock(block);
            }
            this._graphCanvas.addFrame(frameData);
            this.reOrganize(this.props.globalState.nodeParticleSet.editorData, true);
        });
        ...
        this.props.globalState.hostDocument.addEventListener("keydown", (evt) => {
            ...
            void this._graphCanvas.handleKeyDownAsync(
                ...
                async (nodeData) => {
                    const block = nodeData.data as NodeParticleBlock;
    ...
    build(ignoreEditorData = false) {
        ...
        // Load graph of nodes from the particle set
        if (this.props.globalState.nodeParticleSet) {
            this.loadGraph();
        }
    ...
    loadGraph() {
        ...
        for (const system of nodeParticleSet.systemBlocks) {
            this.appendBlock(system, true);
        }

        for (const n of nodeParticleSet.attachedBlocks) {
    ...
    createPopUp = () => {
        ...
        CreatePopup("PREVIEW AREA", {
            ...
            onParentControlCreateCallback: (parentControl) => {
                ...
                parentControl.id = "node-particle-editor-graph-root";
            ...
            onWindowCreateCallback: async (w) => {
                ...
                if (popUpWindow) {
                    ...
                    const parentControl = popUpWindow.document.getElementById("node-particle-editor-graph-root");
    ...
    override render() {
        return (
            <Portal globalState={this.props.globalState}>
                <SplitContainer
                    id="node-particle-editor-graph-root"
                ...
                >
                    ...
                    >
                        <GraphCanvasComponent
                            ...
                            onEmitNewNode={(nodeData) => {
                                return this.appendBlock(nodeData.data as NodeParticleBlock);
                            }}
                        />
                    ...
                    {/* Property tab */}
                    <SplitContainer className="npe-right-panel" direction={SplitDirection.Vertical}>
                        ...
                        <div className="npe-preview-part">
                ...
                <div className="blocker">Node Particle Editor needs a horizontal resolution of at least 900px</div>
