...
export async function fetchParticipantStats(gameId, roster) {
  ...
  const { data, error } = await withTableQuery(supabaseAdmin, 'rank_participants', from =>
    from
...
export async function callPrepareMatchSession({
  ...
  return {
    ...
    data: {
      ...
      sanitizedRoster: Array.isArray(resultRow?.sanitized_roster) ? resultRow.sanitized_roster : [],
    },
  };
}
