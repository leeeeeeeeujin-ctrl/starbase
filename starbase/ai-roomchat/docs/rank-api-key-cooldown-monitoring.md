...
느낀 점: 임계값 감사 로그가 텔레메트리·대시보드에 함께 드러나니 변경 배경을 설명하기가 더 편해져 운영 흐름이 단단해졌습니다.
추가로 필요한 점: 감사 이벤트를 특정 배포나 런북과 연결해 태그할 수 있으면 회고 때 맥락 복원이 더 쉬워질 듯합니다.
진행사항: 임계값 변경 히스토리를 텔레메트리·대시보드 카드로 노출하고 문서에 집계 방식과 활용법을 추가로 정리했습니다.
