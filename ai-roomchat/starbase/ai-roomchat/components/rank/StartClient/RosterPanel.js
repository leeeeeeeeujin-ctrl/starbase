    ...
    >
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        {participants.map(participant => {
          ...
          const status = normalizeTimelineStatus(presenceEntry?.status || participant.status);
          ...
          const isViewerSeat = Boolean(
            viewerKey && ownerId && String(ownerId).trim() === viewerKey
          );
          const isHostRoleSeat = Boolean(hostRoleKey && normalizedRole === hostRoleKey);
          const revealDetails = showDetails || isViewerSeat;
          const heroName = revealDetails ? participant.hero?.name || '이름 없음' : '비공개 참가자';
          ...
          const abilityTexts = revealDetails
            ? [1, 2, 3, 4].map(index => participant.hero?.[`ability${index}`]).filter(Boolean)
            : [];
              ...
              <div style={{ display: 'grid', gap: 8 }}>
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 15 }}>{heroName}</div>
                      ...
                      {(isViewerSeat ||
                        isHostRoleSeat ||
                        (viewerRoleKey && normalizedRole === viewerRoleKey)) && (
