...
| `/rank/[id]/solo` | `RankSoloMatchPage`가 `useGameRoom`으로 접근 권한을 확인한 뒤 `SoloMatchClient`(→ `AutoMatchProgress`)를 띄워 진입 즉시 자동 참가를 시도. | `rank.start.mode`를 `rank_solo`로 유지하고, `selectedHeroId`가 없으면 자동으로 메인 룸으로 되돌아가도록 처리. |
...
| `/rank/[id]/duo/queue` | `DuoMatchQueuePage`가 `useGameRoom`으로 게임 유효성을 확인하고 `DuoMatchClient`(`AutoMatchProgress` 프리셋)를 호출해 듀오 큐에 자동 합류. | 입장 시 `rank.start.mode=rank_duo`를 보존하고, 대기열 이탈 시 언마운트 훅에서 `removeQueueEntry`를 호출해 자동 정리. |
| `/rank/[id]/casual` | `CasualMatchPage`가 `useGameRoom`을 통해 게임 존재 여부를 검증한 뒤 `CasualMatchClient`를 렌더링, 캐주얼 큐에 자동 합류. | 세션에 저장된 `rank.start.turnTimer`/`rank.start.apiVersion` 등을 유지하면서 `casual_match` 모드로 큐를 조회하고, 캐릭터 누락 시 메인 룸으로 복귀. |
