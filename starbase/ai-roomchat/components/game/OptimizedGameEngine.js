...
class BatchProcessor {
  ...
  async processAllBatches() {
    ...
    await Promise.allSettled(
      sessionIds.map(sessionId => this.processBatchForSession(sessionId))
    )
  }
...
export class OptimizedGameEngine {
  ...
  async endGame(reason = 'manual', finalData = {}) {
    ...
    try {
      ...
      // 4. 게임 종료 처리
      let result = 'manual'
      if (reason === 'scoreThreshold' && this.localGameState.score >= (this.config.endConditions.scoreThreshold || 1000)) {
        result = 'win'
