...
export default async function handler(req, res) {
  ...
  const simple =
    typeof rawSimple === 'string' && ['1', 'true', 'yes', 'on'].includes(rawSimple.toLowerCase());
  ...
  try {
    ...
    if (simple) {
      const extendedSelect =
        'id, game_id, room_id, session_id, stage, status, reason, match_code, score_window, mode_col:mode, drop_in, request_id, metadata, created_at';
      const baseSelect =
        'id, game_id, room_id, session_id, stage, status, reason, match_code, score_window, created_at';
      ...
      if (rowsResult?.error) {
        ...
        const isModeAggregate =
          msg.includes('WITHIN GROUP') || msg.toLowerCase().includes('ordered-set');
    ...
    return res
      .status(500)
      .json({ error: 'failed_to_fetch_matchmaking_logs', detail: error?.message || String(error) });
  }
}
