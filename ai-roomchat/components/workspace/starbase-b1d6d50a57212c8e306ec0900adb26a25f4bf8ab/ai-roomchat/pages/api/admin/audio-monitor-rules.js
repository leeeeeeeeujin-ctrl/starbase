...
function sanitiseFilters(raw = {}) {
  ...
  const eventTypes = Array.isArray(raw.eventTypes)
    ? Array.from(
        new Set(raw.eventTypes.map(item => sanitiseString(item, { maxLength: 48 })).filter(Boolean))
      )
    : [];
...
function sanitiseSlack(raw = {}) {
  ...
  const minEvents =
    Number.isFinite(minEventsRaw) && minEventsRaw > 0 ? Math.min(minEventsRaw, 500) : 1;
  const lookbackWeeks =
    Number.isFinite(lookbackRaw) && lookbackRaw > 0 ? Math.min(lookbackRaw, 52) : 4;
