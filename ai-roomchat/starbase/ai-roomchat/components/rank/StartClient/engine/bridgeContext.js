...
export function createBridgeContext({
  ...
  const statusIndex = sessionFlags.statusIndex || buildStatusIndex(participantsStatus, currentRole);
  ...
  const dropInGraceTurns = Number.isFinite(sessionFlags.dropInGraceTurns)
    ...
    : 0;
  const dropInRecent = turnsSinceDropIn != null && turnsSinceDropIn <= dropInGraceTurns;
  ...
  const metrics = {
    win_count: Number.isFinite(sessionFlags.winCount) ? Number(sessionFlags.winCount) : 0,
