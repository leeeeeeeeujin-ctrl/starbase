...
export default function useAdvanceTurn(deps) {
  // Keep a stable ref to the latest deps so the callback can read current
  // values without requiring a huge dependency array. This is a conservative
  // pattern for refactoring large inline callbacks into hooks while avoiding
  // stale-closure bugs and satisfying the linter.
  ...
  return useCallback(
    async (overrideResponse = null, options = {}) => {
      // Read the most-recent deps snapshot
  ...
  // refs / helpers used in body
        ...
        isApiKeyError,
        statusMessageRef,
  // other values that may be referenced by the body
        apiVersionLock,
        recordTurnState,
  ...
  // Support two calling shapes: callers may pass setters individually
  // (e.g. setStatusMessage) or grouped under deps.setters. Tests use the
  // grouped form, so prefer that when available.
  const setters = d.setters || {};
  const setStatusMessage = d.setStatusMessage || setters.setStatusMessage || (() => {});
  ...
  if (!responseText) {
    ...
    // depsRef is stable; callback reads latest values from depsRef.current.
    []
  );
}
