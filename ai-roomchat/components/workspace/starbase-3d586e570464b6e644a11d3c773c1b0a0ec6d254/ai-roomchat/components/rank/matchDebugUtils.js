...
export function summarizeRemovedMembers(entries) {
  ...
  entries.forEach(entry => {
    ...
    const slotKey =
      typeof entry.slotKey === 'string' && entry.slotKey.trim() ? entry.slotKey.trim() : null;
  ...
  assignments.forEach((assignment, assignmentIndex) => {
    ...
    const ensureSummaryEntry = (slotIndex, roleLabel, fallbackIndex) => {
      ...
      let existing = summary.find(
        entry => entry.slotIndex === resolvedIndex && entry.role === resolvedRole
      );
      if (!existing) {
    ...
    if (slotEntries.length > 0) {
      slotEntries.forEach((slot, slotOrdinal) => {
        ...
        const target = ensureSummaryEntry(
          slotIndex,
          roleLabel,
          assignmentIndex * 1000 + slotOrdinal
        );
    ...
    if (removedMembers.length > 0) {
      removedMembers.forEach(entry => {
        const target = ensureSummaryEntry(
          entry.slotIndex,
          entry.role || fallbackRole,
          assignmentIndex * 1000
        );
