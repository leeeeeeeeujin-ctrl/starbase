...
import { clearActiveSessionRecord } from '@/lib/rank/activeSessionStorage';
  ...
  useEffect(() => {
    ...
    async function syncActiveSessionFromServer() {
      try {
        ...
        if (!nextSession || String(nextSession.status || '').trim().toLowerCase() !== 'active') {
          setSession(null);
          clearActiveSessionRecord();
          return;
        }
        ...
        if (cancelled) return;
        console.warn('활성 세션 오버레이 동기화에 실패했습니다:', error);
        setSession(null);
      }
    }
    ...
    const interval = window.setInterval(syncActiveSessionFromServer, 15000);
