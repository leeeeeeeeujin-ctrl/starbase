...
describe('audio monitor rules API', () => {
  ...
  it('rejects unsupported methods', async () => {
    ...
    const req = createApiRequest({
      method: 'PATCH',
      headers: { cookie: createSessionCookie('secret') },
    });
  ...
  it('rejects invalid session cookie', async () => {
    ...
    const req = createApiRequest({
      method: 'GET',
      headers: { cookie: 'rank_admin_portal_session=invalid' },
    });
    const res = createMockResponse();
  ...
  it('returns stored favorites and subscriptions', async () => {
    ...
    const selectOrderSecond = jest.fn(() =>
      Promise.resolve({
        data: [
          {
            id: 'fav-1',
  ...
  it('updates an existing subscription', async () => {
    ...
    const updateChain = {
      eq: jest.fn(() => ({
        select: jest.fn(() => ({
          single: jest.fn(() => Promise.resolve({ data: updatedRecord, error: null })),
        })),
      })),
    };
