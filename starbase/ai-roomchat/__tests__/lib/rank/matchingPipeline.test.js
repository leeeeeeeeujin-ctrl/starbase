...
function createQueueEntry({ id, ownerId, role, score, joinedAt = '2024-01-01T00:00:00Z' }) {
  return {
    id,
    owner_id: ownerId,
    role,
...
function createParticipant({ ownerId, role, score, heroId, updatedAt = '2024-01-01T00:05:00Z' }) {
  return {
    owner_id: ownerId,
    role,
    score,
...
describe('buildCandidateSample', () => {
  it('returns queue entries unchanged when realtime matching is enabled', () => {
    ...
    const participantPool = [createParticipant({ ownerId: 'p3', role: 'attack', score: 1180 })];
  ...
  it('falls back to duplicate-owner stand-ins when no unique candidates exist', () => {
    const queue = [createQueueEntry({ id: 'q1', ownerId: 'creator', role: 'attack', score: 1200 })];
