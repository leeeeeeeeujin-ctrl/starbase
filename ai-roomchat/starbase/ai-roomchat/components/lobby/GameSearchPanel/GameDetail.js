      ...
      <div style={styles.conflictBox}>
        <div style={styles.conflictCard}>
          ...
          <p style={styles.conflictBody}>
            동일 명의로 참가 중인 캐릭터가 있어 새로 입장할 수 없습니다. 아래 정보를 확인한 뒤 기존
            참가 캐릭터로 게임을 진행하거나 다른 게임을 선택해 주세요.
          </p>
          <div style={styles.conflictList}>
            {heroSummaries.map(({ heroName, roleName }, index) => (
              <div
                key={`${heroName}-${roleName || 'none'}-${index}`}
                style={styles.conflictListItem}
              >
          ...
          <p style={styles.conflictHint}>
            참여 중인 캐릭터를 해제하거나 다른 게임을 선택해 주세요.
          </p>
        </div>
      </div>
    );
