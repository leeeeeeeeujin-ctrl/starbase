...
export function useMakerEditor() {
  ...
  const handleAfterSave = useCallback(() => {
    ...
    if (detailCount) {
      console.info('[MakerEditor] 변수 규칙 버전 자동 갱신 완료', {
        count: detailCount,
  ...
  useEffect(() => {
    ...
    try {
      if (saveHistory.length === 0) {
        ...
        window.localStorage.setItem(historyStorageKeyRef.current, JSON.stringify(saveHistory));
      }
    } catch (error) {
        ...
        return [
          ...existing,
          {
            ...
            data: {
              ...
              invisible: false,
              visible_slots: [],
              actor_scope_type: 'all',
              actor_scope_value: '',
              body_visibility_type: 'all',
              body_visibility_value: '',
              guide_visibility_type: 'all',
              guide_visibility_value: '',
              slotNo,
