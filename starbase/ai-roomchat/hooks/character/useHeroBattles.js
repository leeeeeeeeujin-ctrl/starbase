  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      async function fetchByColumn(column) {
        const { data, error: columnError } = await withTable(supabase, 'rank_battles', table =>
          supabase
            .from(table)
            .select(fields)
      ...
      if (!attackRows.length && !defendRows.length) {
        const { data, error: fallbackError } = await withTable(supabase, 'rank_battles', table =>
          supabase
            .from(table)
            .select(fields)
            .eq('game_id', selectedGameId)
        ...
        const fallback = (data || []).filter(
          row =>
            includesHeroId(row.attacker_hero_ids, heroId) ||
            includesHeroId(row.defender_hero_ids, heroId)
        );
        attackRows = fallback;
        defendRows = [];
      }
