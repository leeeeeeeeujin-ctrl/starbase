...
class InputManager {
   ...
   */
  off(action, callback) {
    this.listeners = this.listeners.filter(l => l.action !== action || l.callback !== callback);
  }
  ...
  cleanup() {
    ...
    try {
      if (this.keyboardHandler && this.keyboardHandler.cleanup) this.keyboardHandler.cleanup();
    } catch (_e) {}
    try {
      if (this.touchHandler && this.touchHandler.cleanup) this.touchHandler.cleanup();
    } catch (_e) {}
    try {
      if (this.gamepadHandler && this.gamepadHandler.cleanup) this.gamepadHandler.cleanup();
    } catch (_e) {}
