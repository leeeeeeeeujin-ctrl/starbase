...
- **운영용 크론 파이프라인**: `vercel.json`에 `/api/rank/cooldown-digest` 크론을 추가해 매시간 미알림 이벤트를 수집·로그로 남기고, 처리된 항목은 `notified_at` 타임스탬프로 표시한다.【F:vercel.json†L1-L9】【F:pages/api/rank/cooldown-digest.js†L1-L90】
- **운영 가이드**: 새 `rank-api-key-cooldown-monitoring.md` 문서가 테이블 생성, 환경 변수, Cron 운용 절차를 요약해 운영팀이 바로 참고할 수 있도록 정리했다.【F:docs/rank-api-key-cooldown-monitoring.md†L1-L88】
