...
### 텍스트 배틀 / 랭크 수직선 구현 단계 (상세 계획)

> 이 하위 섹션은 실제 구현/테스트 순서를 정리한 실행 계획입니다.  
> 위의 A–E TODO 항목을 “어떤 순서로, 어느 수준까지” 끌어올릴지에 대한 체크리스트로 사용합니다.

- **1단계: 싱글 플레이 수직선 완성 (Maker → Play → Rank 메인게임 → 정산)**
  - Maker/워크스페이스에서 텍스트 배틀 세트 하나를 만든 뒤:
    - Play(builtin 텍스트 런타임)에서 첫 턴부터 onBattleEnd까지 자연스럽게 도는지 확인합니다.
    - 동일 세트가 랭크 게임의 StartClient에서도 그대로 실행되도록,  
      `StartClient` 의 텍스트 런타임 경로를 Maker/Play와 동일한 엔진/그래프/훅 조합으로 고정합니다.
  - AI 호출:
    - 메인게임 텍스트 런타임이 `/api/ai/gemini` 를 호출할 때,  
      Play 디버그와 동일한 방식으로 “유저 API 키/키링”을 우선 사용하고,  
      키가 없거나 오류인 경우에는 **명시적인 시스템 에러 메시지**를 띄우고 턴을 중단합니다.
    - 이 단계의 완료 기준: **혼자 매칭을 걸어도** 한 판이 끝까지 돌아가고,  
      최소한 `battleLast.narrative` 기반 내레이션이 매 턴/종료 시점에 보이는 것입니다.

- **2단계: 메인게임 ↔ 텍스트 배틀 로그/정산 연결**
  - onBattleEnd 훅에서:
    - `text_battle_sessions` / `text_battle_turns` 행을 생성하고,
    - 랭크 정산(`/api/rank/settle`)과 함께 텍스트 배틀 요약을 전달합니다.
  - 정산 후:
    - 히스토리/전적 뷰에서 해당 판의 텍스트 배틀 로그를 열어 볼 수 있는 링크를 노출합니다.
  - 완료 기준: **한 판을 끝까지 돌린 뒤**,  
    - Supabase `text_battle_sessions` / `text_battle_turns` 에 기록이 남고,
    - 랭크 점수/승패도 기존 규칙대로 정산되는 것입니다.

- **3단계: 멀티 플레이/12인 기준 매칭과 수명 관리**
  - `rank_match_queue` 를 기준으로:
    - 매칭이 성사되면 티켓이 다시 소비되지 않도록 하는 RPC/트리거를 설계·구현합니다.
    - 최대 12인(예: 6:6, 12인 방)까지 한 번에 방을 구성하는 범용 매칭 흐름을 정리합니다.
  - 매칭이 만들어 낸 방/세션 정보는:
    - `rank_sessions` / `rank_match_roster` 로 수렴시키고,
    - StartClient 의 `rankContext` / `matchState` 에서 동일 정보를 읽어  
      텍스트 런타임과 UI가 같은 참가자/점수/턴 상태를 공유하도록 맞춥니다.
  - 완료 기준:
    - 둘 이상 클라이언트가 동일 게임에 매칭되었을 때,
    - 캐릭터 패널/점수/턴 진행 상태가 서로 동기화된 채로 같은 텍스트 배틀을 공유하는 것입니다.

- **4단계: 운영/디버그 도구 및 안전장치**
  - `/tools/rank-session-debug` (또는 유사 경로)에서:
    - `fetch_latest_rank_session_v2` / `cleanup_expired_rank_sessions` / 텍스트 배틀 세션 로그를  
      한눈에 볼 수 있는 UI를 제공합니다.
  - `cleanup_expired_rank_sessions` 와 `rank_match_queue` 정리 로직은:
    - “예전 세션으로 빨려 들어가는” 현상이 다시 생기지 않도록,  
    - 위 디버그 도구에서 직접 상태를 확인하고 수동/자동 정리를 걸 수 있게 설계합니다.
