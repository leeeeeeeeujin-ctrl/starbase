...
export async function publishPromptSetToLibrary(userId, setId) {
  ...
  const { data: setRow, error: setError } = await withTableQuery(supabase, 'prompt_sets', from =>
    from.select('*').eq('id', setId).single()
  );
...
export async function importPromptLibraryEntry(userId, entryId) {
  ...
  const { data: entry, error } = await withTableQuery(supabase, 'prompt_library_entries', from =>
