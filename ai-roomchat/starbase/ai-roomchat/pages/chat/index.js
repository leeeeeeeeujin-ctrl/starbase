...
const LAYOUT = {
  ...
  messageViewport: (height = null) => ({
    ...
    height: height ? `${Math.round(height)}px` : '100%',
    minHeight: 220,
    maxHeight: '100%',
    overflowY: 'auto',
    padding: '0 4px 16px',
  ...
  messageResizer: (active = false) => ({
    ...
    userSelect: 'none',
    borderRadius: 12,
    border: active ? '1px solid rgba(148, 163, 184, 0.45)' : '1px solid rgba(148, 163, 184, 0.18)',
    background: active ? 'rgba(59, 130, 246, 0.12)' : 'rgba(15, 23, 42, 0.45)',
    transition: 'background 0.2s ease, border 0.2s ease',
  }),
  messageResizerGrip: (active = false) => ({
    width: 64,
    height: 6,
    borderRadius: 999,
    background: active ? 'rgba(191, 219, 254, 0.9)' : 'rgba(148, 163, 184, 0.4)',
  ...
  messageBubble: (mine = false) => ({
    ...
    boxShadow: mine
      ? '0 14px 36px -28px rgba(59, 130, 246, 0.7)'
      : '0 14px 36px -28px rgba(15, 23, 42, 0.7)',
  }),
  messageText: {
    fontSize: 13,
    lineHeight: 1.45,
  ...
  notice: (active = false) => ({
    padding: '12px 16px',
    borderRadius: 16,
    border: active ? '1px solid rgba(59, 130, 246, 0.32)' : '1px solid rgba(248, 113, 113, 0.25)',
...
function resolveViewerParticipant(session, viewer) {
  ...
  return participants.find(entry => normalizeId(entry.owner_id) === viewerId) || null;
}
...
export default function ChatPage() {
  ...
  const viewportResizeStateRef = useRef({ teardown: null });
  ...
  const subscriptionKey = useMemo(() => {
    ...
    if (context.sessionId)
      return `messages-session-${context.sessionId}-${context.scope || 'main'}`;
  ...
  const beginViewportResize = useCallback(
    event => {
      ...
      const viewportEl = messageListRef.current;
      ...
      const shell = viewportEl.parentElement;
      const panel = shell?.parentElement;
      const composerEstimate = 140;
      const shellHeight = shell?.offsetHeight || viewportEl.offsetHeight || 0;
      const panelHeight = panel?.offsetHeight || shellHeight + composerEstimate;
      const minHeight = 220;
      const maxHeight = Math.max(minHeight, panelHeight - composerEstimate);
      const startHeight = viewportHeight ?? shellHeight;
      ...
      const stopResize = () => {
        ...
        viewportResizeStateRef.current.teardown = null;
      };

      viewportResizeStateRef.current.teardown = stopResize;
  ...
  const composerNotice = useMemo(() => {
    ...
    if (
      (context.scope === 'main' || context.scope === 'role') &&
      (sessionWindow.notice || sessionWindow.hint)
    ) {
      return {
        ...
        const inserted = await insertMessage(
          {
            ...
            target_role:
              context.scope === 'role' ? context.viewerRole || context.targetRole || null : null,
          },
          {
  ...
  const handleCreateRoom = useCallback(
    async (event, formState, resetForm, tabHint = 'my-rooms') => {
      event?.preventDefault?.();
      if (!formState) return;
      const { name, description, visibility, capacity, allowAi, requireApproval, heroId } =
        formState;
          ...
          <div style={contentStyle}>
                        ...
                        <section style={LAYOUT.card}>
                          ...
                          {activeHero ? (
                            <p style={{ margin: 0, color: '#94a3b8', fontSize: 13 }}>
                              {activeHero.description || '선택한 영웅으로 메시지를 전송합니다.'}
                            </p>
                          ) : (
                              ...
                              <article key={session.session_id} style={LAYOUT.card}>
                                ...
                                >
                                  <button
                                    ...
                                    onClick={() =>
                                      handleSelectContext(
                                        buildSessionContext(session, 'main', viewer),
                                        'info'
                                      )
                                    }
                                  >
                                  ...
                                  {session.viewer_role ? (
                                    <button
                                      ...
                                      onClick={() =>
                                        handleSelectContext(
                                          buildSessionContext(session, 'role', viewer),
                                          'info'
                                        )
                                      }
                                    >
                            ...
                            contacts.map(contact => {
                              ...
                              return (
                                <button
                                  ...
                                  type="button"
                                  style={LAYOUT.smallButton(
                                    context?.scope === 'whisper' &&
                                      context?.targetHeroId === ctx.targetHeroId
                                  )}
                              ...
                              <article key={room.id} style={LAYOUT.card}>
                                ...
                                >
                                  <button
                                    ...
                                    onClick={() =>
                                      handleSelectContext(buildRoomContext(room), 'my-rooms')
                                    }
                                  >
                                    대화 열기
                                  </button>
                    ...
                    {activeTab === 'open-search' ? (
                      <>
                        ...
                        >
                          ...
                          <button
                            type="submit"
                            style={LAYOUT.button('primary', searchBusy)}
                            disabled={searchBusy}
                          >
                            {searchBusy ? '검색 중…' : '검색'}
                          </button>
            ...
            {isChatActive ? (
              <section style={LAYOUT.messagePanel}>
                <div style={LAYOUT.messageHeader}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ display: 'grid', gap: 4 }}>
                      ...
                      >
                        ...
                        {context?.scope === 'role' && context?.viewerRole ? (
                          <span>{context.viewerRole}</span>
                        ) : null}
                        {context?.memberCount != null ? (
                          <span>{context.memberCount}명 참여</span>
                        ) : null}
                      </div>
                    </div>
                  </div>
                ...
                <div style={LAYOUT.messageViewportShell}>
                  <div ref={messageListRef} style={LAYOUT.messageViewport(viewportHeight)}>
                    {loadingMessages ? (
                        ...
                        return (
                          <div key={message.id} style={LAYOUT.messageRow(mine)}>
                            ...
                            {mine ? (
                              ...
                              <span style={LAYOUT.messageTimestamp(false)}>{createdAt}</span>
                            )}
                          </div>
                        );
                      })
                  ...
                  <div
                    ...
                    style={LAYOUT.messageResizer(resizingViewport)}
                  >
                    <span style={LAYOUT.messageResizerGrip(resizingViewport)} />
                  </div>
                </div>
                <div style={LAYOUT.composer} data-composer-anchor>
                  {composerNotice ? (
                  ...
                  <form onSubmit={handleSendMessage} style={LAYOUT.composerRow}>
                    ...
                    <button
                      type="submit"
                      style={LAYOUT.button('primary', messageComposerDisabled)}
                      disabled={messageComposerDisabled}
                    >
                      전송
                    </button>
                    <button
                      ...
                      style={LAYOUT.button(
                        'ghost',
                        !hasActiveKey || aiBusy || messageComposerDisabled
                      )}
