...
function buildLocalSampleMeta({
  ...
  const queueSampled =
    sampleType === 'realtime_queue' ? sampleCount : Math.min(queueCount, sampleCount);
...
export default function useLocalMatchPlanner({ gameId, mode, enabled }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setSampleType(
        sampleSet.sampleType || (sampleSet.realtimeEnabled ? 'realtime_queue' : 'participant_pool')
      );
      ...
      const metaPayload = buildLocalSampleMeta({
        ...
        sampleType:
          ...
          (sampleSet.realtimeEnabled ? 'realtime_queue' : 'participant_pool'),
