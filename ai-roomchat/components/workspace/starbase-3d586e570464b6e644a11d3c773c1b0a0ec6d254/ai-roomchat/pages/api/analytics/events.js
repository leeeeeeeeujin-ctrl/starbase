...
async function upsertSession(supabase, sessionId, sessionData) {
  ...
  const { error } = await supabase.from('analytics_sessions').upsert(sessionRecord, {
...
async function saveGameEvents(supabase, events) {
  ...
  const { error } = await supabase.from('analytics_game_events').insert(gameEventRecords);
...
async function saveUserActions(supabase, actions) {
  ...
  const { error } = await supabase.from('analytics_user_actions').insert(actionRecords);
...
async function savePerformanceMetrics(supabase, metrics) {
  ...
  const { error } = await supabase.from('analytics_performance_metrics').insert(metricRecords);
...
async function saveErrors(supabase, errors) {
  ...
  const { error } = await supabase.from('analytics_errors').insert(errorRecords);
...
async function saveAIRequests(supabase, requests) {
  ...
  const { error } = await supabase.from('analytics_ai_requests').insert(requestRecords);
...
async function updateRealtimeStats(supabase, sessionId, results) {
  try {
    ...
    if (existingStats) {
      ...
      const { error } = await supabase.from('analytics_daily_stats').insert(statsUpdate);

      if (error) throw error;
    }

    // 실시간 채널로 통계 브로드캐스트
    await supabase.channel('analytics_updates').send({
      type: 'broadcast',
      event: 'stats_update',
      payload: {
        sessionId,
