...
4. **Telemetry 리포트 & 대시보드**
  ...
  - `thresholdAudit.timeline`은 최근 14일(최대 30일) 감사 이벤트를 UTC 일 단위로 버킷팅해 `{ start, end, label, weekday, count, isToday }` 배열을 반환합니다. `maxCount`와 `windowDays`를 활용해 UI 차트의 축을 계산합니다.
  ...
  - 임계값 감사 패널에는 최근 14일 감사 이벤트를 세로 막대로 표현한 타임라인 그래프가 추가돼 배포/임계값 조정 빈도 추세를 한눈에 비교할 수 있습니다. 당일 이벤트는 하이라이트 색상으로 표시됩니다.
...
느낀 점: 감사 이벤트 타임라인이 붙으니 최근 변경 추세를 한눈에 짚을 수 있어 운영 보고 준비가 훨씬 수월해졌습니다.
추가로 필요한 점: 타임라인 버킷을 주간/월간으로 전환하는 옵션이 생기면 배포 밀집 기간을 더 직관적으로 비교할 수 있을 듯합니다.
진행사항: 임계값 감사 타임라인을 텔레메트리·대시보드에 추가하고 문서에 버킷 구조·시각화 방식을 반영했습니다.
