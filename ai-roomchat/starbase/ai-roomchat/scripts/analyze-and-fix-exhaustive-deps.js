...
function readJson(p) {
  try {
    ...
    console.error('Failed read', p, e.message);
    process.exit(2);
  }
}
...
function shouldSkip(file) {
  return SKIP_DIRS.some(
    d =>
      file.includes(path.sep + d + path.sep) || file.includes(path.posix.sep + d + path.posix.sep)
  );
}
...
function run() {
  ...
  const entries = data.flatMap(entry =>
    (entry.messages || [])
      ...
      .map(m => ({ file: entry.filePath, line: m.line }))
  );
  const byFile = {};
  ...
  for (const [file, linesSet] of Object.entries(byFile)) {
    ...
    if (!fs.existsSync(file)) {
      report.recommendations.push({ file, reason: 'missing file' });
      continue;
    }
    ...
    for (const reportedLine of linesArr) {
      ...
      if (!deps) {
        report.recommendations.push({ file, line: reportedLine, reason: 'deps not found' });
        continue;
      }
      const depArray = normalizeDeps(deps.raw);
      const hookStart = findHookStart(lines, deps.idx);
      if (hookStart === null) {
        report.recommendations.push({ file, line: reportedLine, reason: 'hook start not found' });
        continue;
      }
      ...
      const filtered = missing.filter(
        id => !['props', 'state', 'setState', 'dispatch'].includes(id)
      );
