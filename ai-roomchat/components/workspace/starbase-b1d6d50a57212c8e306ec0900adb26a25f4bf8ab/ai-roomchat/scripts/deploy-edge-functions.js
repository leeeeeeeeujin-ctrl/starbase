...
const SLACK_NOTIFY_ON_SUCCESS =
  (process.env.RANK_EDGE_DEPLOY_SLACK_NOTIFY_SUCCESS || '').toLowerCase() === 'true';

const PAGERDUTY_ROUTING_KEY = sanitiseString(process.env.RANK_EDGE_DEPLOY_PAGERDUTY_ROUTING_KEY);
const PAGERDUTY_SEVERITY =
  sanitiseString(process.env.RANK_EDGE_DEPLOY_PAGERDUTY_SEVERITY) || 'critical';
const PAGERDUTY_SOURCE =
  sanitiseString(process.env.RANK_EDGE_DEPLOY_PAGERDUTY_SOURCE) || 'rank-edge-deploy';

const FUNCTIONS = parseList(process.env.RANK_EDGE_DEPLOY_FUNCTIONS, DEFAULT_FUNCTIONS);
const MAX_ATTEMPTS = toPositiveInteger(
  process.env.RANK_EDGE_DEPLOY_MAX_ATTEMPTS,
  DEFAULT_MAX_ATTEMPTS
);
const BASE_RETRY_MS = toPositiveInteger(
  process.env.RANK_EDGE_DEPLOY_BASE_RETRY_MS,
...
const SMOKE_TEST_METHOD = (
  sanitiseString(process.env.RANK_EDGE_DEPLOY_SMOKE_TEST_METHOD) || 'GET'
).toUpperCase();
...
function parseHeaders(value) {
  ...
  try {
    ...
    console.warn('[edge-deploy] Failed to parse SMOKE_TEST_HEADERS JSON. Ignoring headers.', {
      error,
    });
    return {};
  }
...
async function postSlack(payload) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Slack webhook responded with ${response.status} ${response.statusText}: ${detail}`
      );
    }
    return true;
  } catch (error) {
...
async function notifyPagerDuty({
  ...
  try {
    ...
    if (!response.ok) {
      const detail = await response.text().catch(() => '');
      throw new Error(
        `PagerDuty responded with ${response.status} ${response.statusText}: ${detail}`
      );
    }
    return true;
  } catch (error) {
  ...
  for (let index = 0; index < SMOKE_TEST_URLS.length; index += 1) {
    ...
    try {
      ...
      const bodyText = await response
        ...
        .catch(() => '')
        .then(text =>
          text && text.length > MAX_LOG_CHARS ? `${text.slice(0, MAX_LOG_CHARS - 3)}...` : text
        );
    ...
    while (attempt < MAX_ATTEMPTS && !succeeded) {
      ...
      if (!success) {
        ...
        if (status === 'failed') {
          ...
          console.log(
            `[edge-deploy] Waiting ${Math.round(delayMs / 1000)}s before retrying ${fnName}`
          );
          await delay(delayMs);
        }
      } else {
