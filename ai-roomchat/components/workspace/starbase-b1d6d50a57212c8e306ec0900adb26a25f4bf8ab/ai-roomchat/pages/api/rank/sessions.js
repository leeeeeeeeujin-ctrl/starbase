...
export default async function handler(req, res) {
  ...
  const {
    gameId,
    limit: rawLimit,
    turnLimit: rawTurnLimit,
    timelineLimit: rawTimelineLimit,
  } = req.query || {};
  ...
  if (sessionIds.length) {
    const { data: turnRows, error: turnError } = await withTableQuery(
      ...
      from =>
        from
          .select(
            'id, session_id, idx, role, public, is_visible, content, summary_payload, created_at'
          )
  ...
  const payload = sessions.map(session => {
    ...
    const hiddenPrivateCount = viewerIsOwner ? 0 : rawTurns.length - visibleTurns.length;
  ...
  const nextCursor =
    sessions.length === limit ? (sessions[sessions.length - 1]?.created_at ?? null) : null;
