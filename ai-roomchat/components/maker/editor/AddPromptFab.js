    ...
    <div data-overlay="prompt-fab" style={{ position: 'fixed', right: 24, bottom: 18, zIndex: 240 }}>
      <div style={{ position: 'relative', width: 58, height: 58 }}>
        ...
        <div style={{ position: 'absolute', right: 0, bottom: 68, display: 'grid', gap: 8 }}>
          <button style={{ ...btn, background: '#eef2ff', color: '#3730a3', borderColor: '#c7d2fe' }} onClick={() => { onAdd?.('ai',''); setOpen(false); }}>
            AI 턴 추가
            <div style={{ marginTop: 4, fontSize: 11, fontWeight: 500, color: 'rgba(55,48,163,0.72)' }}>AI에게 보낼 프롬프트와 응답 흐름</div>
          </button>
          <button style={{ ...btn, background: '#e0f2fe', color: '#075985', borderColor: '#bae6fd' }} onClick={() => { onAdd?.('user_action',''); setOpen(false); }}>
            유저 입력 턴 추가
            <div style={{ marginTop: 4, fontSize: 11, fontWeight: 500, color: 'rgba(7,89,133,0.78)' }}>입력창, 선택, 제출 결과 저장</div>
          </button>
          <button style={{ ...btn, background: '#fee2e2', color: '#991b1b', borderColor: '#fecaca' }} onClick={() => { onAdd?.('system',''); setOpen(false); }}>
            시스템 턴 추가
