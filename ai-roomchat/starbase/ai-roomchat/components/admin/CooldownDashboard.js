    ...
    try {
      ...
      try {
        const uploadResult = await uploadBlobToTeamDrive({
          blob,
          filename,
          metadata: { section: 'audit-timeline', mode: auditTimelineMode, format: 'csv', exportedAt: timestamp },
        })
        if (uploadResult.status === 'uploaded') {
          uploadSuffix = ' · 팀 드라이브 업로드 완료'
        } else if (uploadResult.status === 'skipped') {
  ...
  const timelineUploadHelper = useMemo(() => {
    ...
    return joinHelper([
      ...
      metrics.failureStreak > 0
        ? `연속 실패 ${formatNumber(metrics.failureStreak)}회`
        : null,
          ...
          <section className={styles.panel}>
            ...
            <dl className={styles.thresholdList}>
              ...
              <div>
                <dt>업로드 연속 실패</dt>
                <dd>
                  주의 ≥ {formatNumber(telemetry.alerts?.thresholds?.timelineUploadFailureStreak?.warning ?? null)}회 · 위험 ≥{' '}
                  {formatNumber(telemetry.alerts?.thresholds?.timelineUploadFailureStreak?.critical ?? null)}회
                </dd>
              </div>
              <div>
                <dt>업로드 성공 간격</dt>
                <dd>
                  주의 ≥ {formatNumber(telemetry.alerts?.thresholds?.timelineUploadStaleHours?.warning ?? null, { digits: 0 })}시간 · 위험 ≥{' '}
                  {formatNumber(telemetry.alerts?.thresholds?.timelineUploadStaleHours?.critical ?? null, { digits: 0 })}시간
                </dd>
              </div>
              <div>
                <dt>런북 링크 첨부율</dt>
