...
export function buildCooldownTelemetry(rows = [], { latestLimit = 15 } = {}) {
  ...
  const providerSummaries = Array.from(providerMap.values()).map((entry) => {
    ...
    const avgRotationDurationMs = averageFrom(entry.rotationDurationSum, entry.rotationDurationCount)

    const combinedDurations = [avgAlertDurationMs, avgRotationDurationMs].filter((value) => value !== null)
    const avgDuration = combinedDurations.length
      ? Math.round(combinedDurations.reduce((sum, value) => sum + value, 0) / combinedDurations.length)
      : null
  ...
  const avgRotationDurationMs = averageFrom(totals.rotationDurationSum, totals.rotationDurationCount)
  const combinedDurations = [avgAlertDurationMs, avgRotationDurationMs].filter((value) => value !== null)
  const avgDuration = combinedDurations.length
    ? Math.round(combinedDurations.reduce((sum, value) => sum + value, 0) / combinedDurations.length)
    : null
