...
function SectionList({ rows, emptyMessage, renderAside, renderMeta }) {
  if (!rows.length) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        battleData.forEach(battle => {
          const attackerHeroes = Array.isArray(battle.attacker_hero_ids)
            ? battle.attacker_hero_ids
            : [];
          const defenderHeroes = Array.isArray(battle.defender_hero_ids)
            ? battle.defender_hero_ids
            : [];
        ...
        <div className={styles.content}>
          ...
          <div className={styles.section}>
            <div className={styles.sectionHeader}>
              ...
              {activeSeason ? (
                <span className={styles.badge}>진행 중 · {activeSeason.name}</span>
              ) : null}
            </div>
            {hasSeasonContent ? (
              <>
                  ...
                  renderMeta={entry =>
                    [
                      ...
                      entry.rating != null ? (
                        <span key="rating">
                          레이팅{' '}
                          <span className={styles.rowStat}>{formatNumber(entry.rating)}</span>
                        </span>
                      ) : null,
                      entry.score != null ? (
                        <span key="score">
            ...
            <SectionList
              ...
              renderMeta={entry =>
                [
                  ...
                  entry.battles != null ? (
                    <span key="battles">전투 {formatNumber(entry.battles)}</span>
                  ) : null,
                ].filter(Boolean)
              }
              renderAside={entry =>
                entry.updatedAt
                  ? [<span key="updated">갱신 {formatDateLabel(entry.updatedAt)}</span>]
                  : null
              }
            />
          </div>
