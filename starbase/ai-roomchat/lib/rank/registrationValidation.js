...
function sanitizeRole(role) {
  const baseName = typeof role?.name === 'string' ? role.name.trim() : ''
  const name = baseName || '역할'

  const slotCount = Math.max(0, Math.round(toFiniteNumber(role?.slot_count, 0)))
...
export function prepareRegistrationPayload(raw) {
  ...
  const sanitizedRoles = roleList.map(sanitizeRole)
  ...
  const rulesPrefix = typeof raw.rules_prefix === 'string' && raw.rules_prefix.trim()
