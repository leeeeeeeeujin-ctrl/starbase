...
async function uploadBlobToTeamDrive({ blob, filename, metadata }) {
  if (typeof window === 'undefined')
    return { status: 'skipped', message: '클라이언트가 아닙니다.' };
  ...
  const response = await fetch('/api/rank/upload-cooldown-timeline', {
    method: 'POST',
  ...
  if (response.status === 202) {
    return { status: 'skipped', message: '팀 드라이브 업로드 설정이 없어 건너뛰었습니다.' };
  }

  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throw new Error(payload.message || '팀 드라이브 업로드에 실패했습니다.');
  }

  const payload = await response.json().catch(() => ({}));
  return { status: 'uploaded', payload };
}
...
function formatRelativeTimestamp(isoString) {
  ...
  const baseLabel = timestamp.toLocaleString(
    'ko-KR',
    sameDay
      ? { hour: '2-digit', minute: '2-digit' }
      : { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }
    ...
    <article className={styles.gaugeCard}>
      ...
      <div className={styles.gaugeBar}>
        ...
        {normalizedWarning !== null && (
          <div
            className={`${styles.gaugeThreshold} ${styles.gaugeThresholdWarning}`}
            style={{ left: `${normalizedWarning * 100}%` }}
          />
        )}
        {normalizedCritical !== null && (
          <div
            className={`${styles.gaugeThreshold} ${styles.gaugeThresholdCritical}`}
            style={{ left: `${normalizedCritical * 100}%` }}
          />
        )}
      </div>
                  ...
                  {items.map((item, index) => (
                    <li key={`${tier}-${index}`} className={styles.tierItem}>
                      ...
                      <p className={styles.tierMeta}>
                        {formatNumber(item.matches)}회 · 승률 {formatPercent(item.winRate)} · Δ
                        {formatDeltaPercent(item.delta)}
                      </p>
                    </li>
                  ))}
        ...
        return (
          <li key={event.id} className={styles.auditItem}>
            ...
            <p className={styles.auditSummary}>
              {event.summary || '변경 내역을 요약하지 못했습니다.'}
            </p>
            {event.diff?.length ? (
...
function ThresholdAuditTimeline({ timeline, options = [], activeMode, onModeChange }) {
  ...
  const maxCount =
    timeline.maxCount ?? Math.max(...timeline.buckets.map(bucket => bucket.count || 0), 0);
  const legendLabel =
    timeline.windowLabel ||
    (timeline.windowDays ? `최근 ${formatNumber(timeline.windowDays)}일` : null);
...
function TimelineUploadGroupCard({ group }) {
  ...
  const lastEventLabel = lastEventTimestamp
    ? formatRelativeTimestamp(lastEventTimestamp)
    : '기록 없음';
  const lastSuccessLabel = group.lastSuccessAt
    ? formatRelativeTimestamp(group.lastSuccessAt)
    : '기록 없음';
            ...
            return (
              <li key={formatKey} className={styles.timelineUploadFormatItem}>
                ...
                <span className={styles.timelineUploadFormatDetail}>
                  24시간 {formatNumber(formatSummary.success24h)}회 · 7일{' '}
                  {formatNumber(formatSummary.success7d)}회
                </span>
              </li>
            );
          })}
  ...
  return (
    <ul className={styles.timelineUploadList}>
      {entries.map(entry => {
        ...
        const key =
          entry.id ||
          `${entry.section || 'entry'}-${entry.insertedAt || entry.uploadedAt || Math.random()}`;
          ...
          <li key={key} className={styles.timelineUploadItem}>
            <header className={styles.timelineUploadItemHeader}>
              <div>
                ...
                {entry.filename ? (
                  <span className={styles.timelineUploadFilename}> · {entry.filename}</span>
                ) : null}
              </div>
              {statusInfo ? (
                <span className={`${styles.uploadStatus} ${statusInfo.tone}`}>
                  {statusInfo.label}
                </span>
              ) : null}
            </header>
      ...
      {attempts.map((attempt, index) => (
        <li key={`${attempt.keyHash || 'unknown'}-${index}`} className={styles.attemptItem}>
          ...
          <p className={styles.attemptMeta}>
            ...
            {attempt.attemptedAt
              ? new Date(attempt.attemptedAt).toLocaleString('ko-KR')
              : '시각 미기록'}
          </p>
          ...
          {typeof attempt.docLinkAttached === 'boolean' && (
            <p className={styles.attemptMeta}>
              ...
              {typeof attempt.docLinkAttachmentCount === 'number' &&
...
export default function CooldownDashboard() {
  ...
  const activeAuditTimeline = useMemo(() => {
    ...
    telemetry?.thresholdAudit?.timelines,
    telemetry?.thresholdAudit?.timeline,
    auditTimelineMode,
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      window.localStorage.setItem(LANGUAGE_FAVORITES_STORAGE_KEY, JSON.stringify(filterFavorites));
    } catch (storageError) {
  ...
  const handleExport = useCallback(
    async section => {
      ...
      try {
        ...
        const filename = `cooldown-${section}-${timestamp}.csv`;
        ...
        let uploadSuffix = '';
        try {
          const uploadResult = await uploadBlobToTeamDrive({
            blob,
            filename,
            metadata: { section, format: 'csv', exportedAt: timestamp },
          });
          if (uploadResult.status === 'uploaded') {
            uploadSuffix = ' · 팀 드라이브 업로드 완료';
          } else if (uploadResult.status === 'skipped') {
            uploadSuffix = ' · 팀 드라이브 업로드 건너뜀';
          }
        } catch (uploadError) {
          uploadSuffix = ` · 업로드 실패: ${uploadError.message}`;
          setExportStatus({
            section,
            type: 'error',
            text: `CSV 파일을 다운로드했지만 ${uploadSuffix.slice(3)}`,
          });
          return;
        }
        ...
        setExportStatus({
          section,
          type: 'success',
          text: `CSV 파일을 다운로드했습니다${uploadSuffix}.`,
        });
      } catch (exportError) {
        setExportStatus({
          section,
  ...
  const renderExportFeedback = section => {
    ...
    const toneClass =
      exportStatus.type === 'success' ? styles.exportFeedbackSuccess : styles.exportFeedbackError;
  ...
  const handleTimelineExportCsv = useCallback(async () => {
    ...
    try {
      ...
      const blob = await response.blob();
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const filename = `cooldown-threshold-audit-${sanitizeFilenameSegment(
        auditTimelineMode
      )}-${timestamp}.csv`;
      const downloadUrl = window.URL.createObjectURL(blob);
      const anchor = document.createElement('a');
      ...
      try {
        ...
        setTimelineExportStatus({
          type: 'error',
          text: `타임라인 CSV를 다운로드했지만 업로드에 실패했습니다: ${uploadError.message}`,
        });
        return;
      }
      ...
      setTimelineExportStatus({
        type: 'success',
        text: `타임라인 CSV 파일을 다운로드했습니다${uploadSuffix}.`,
      });
    } catch (timelineError) {
      setTimelineExportStatus({
        type: 'error',
  ...
  const handleTimelineExportImage = useCallback(async () => {
    ...
    try {
      ...
      const gridSteps = Math.min(
        5,
        Math.max(2, Math.ceil(maxCount / Math.max(1, Math.floor(maxCount / 4))))
      );
      ...
      buckets.forEach((bucket, index) => {
        ...
        ctx.fillText(
          String(value),
          x + barWidth / 2,
          Math.min(chartBottom - barHeight - 8, chartBottom - chartHeight + 16)
        );
        ctx.restore();
      ...
      ctx.font = '500 12px "Noto Sans KR", "Pretendard", sans-serif';
      ctx.fillText(
        '타임라인 값은 감사 이벤트 발생 횟수를 기준으로 집계됩니다.',
        leftPadding,
        height - 24
      );
      ...
      try {
        ...
        setTimelineExportStatus({
          type: 'error',
          text: `타임라인 이미지를 저장했지만 업로드에 실패했습니다: ${uploadError.message}`,
        });
        return;
      }
      ...
      setTimelineExportStatus({
        type: 'success',
        text: `타임라인 이미지를 저장했습니다${uploadSuffix}.`,
      });
    } catch (timelineError) {
      setTimelineExportStatus({
        type: 'error',
  ...
  const renderTimelineExportFeedback = () => {
    ...
    return (
      <p
        className={`${styles.exportFeedback} ${toneClass} ${styles.timelineExportFeedback}`}
        role="status"
      >
  ...
  function describeFavorite(favorite) {
    ...
    return seasonLabel
      ? `${limitLabel} · ${gameLabel} · ${seasonLabel}`
      : `${limitLabel} · ${gameLabel}`;
  }
  ...
  function handleApplyFavorite(favorite) {
    ...
    setSelectedSeasonId(
      favorite.gameId && favorite.gameId !== 'all' && favorite.seasonId ? favorite.seasonId : 'all'
    );
  ...
  const timelineUploadStatus = timelineUploadAlert.status || 'ok';
  ...
  const timelineUploadHelper = useMemo(() => {
    ...
    const metrics = timelineUploadAlert.metrics || {};
    return joinHelper([
      ...
      timelineUploadOverall.lastSuccessAt
        ...
        : '최근 성공 없음',
      metrics.failureStreak > 0 ? `연속 실패 ${formatNumber(metrics.failureStreak)}회` : null,
      typeof metrics.hoursSinceLastSuccess === 'number'
        ? `${formatNumber(metrics.hoursSinceLastSuccess, { digits: 1 })}시간 경과`
        : null,
    ]);
  }, [timelineUploadAlert, timelineUploadOverall]);
    ...
    async function loadInsights() {
      ...
      try {
        ...
        if (!response.ok) {
          const payload = await response
            .json()
            .catch(() => ({ error: '언어 인사이트를 불러오지 못했습니다.' }));
      ...
      {loading ? (
        ...
        <>
          ...
          <section className={styles.panel}>
            ...
            <dl className={styles.thresholdList}>
              <div>
                <dt>실패 비율</dt>
                <dd>
                  주의 {telemetry.alerts?.thresholds?.failureRate?.warning ?? '—'} · 위험{' '}
                  {telemetry.alerts?.thresholds?.failureRate?.critical ?? '—'}
                </dd>
              </div>
              <div>
                <dt>쿨다운 비중</dt>
                <dd>
                  주의 {telemetry.alerts?.thresholds?.triggeredRatio?.warning ?? '—'} · 위험{' '}
                  {telemetry.alerts?.thresholds?.triggeredRatio?.critical ?? '—'}
                </dd>
              </div>
              <div>
                <dt>알림 소요 시간</dt>
                <dd>
                  주의 {formatDuration(telemetry.alerts?.thresholds?.avgAlertDurationMs?.warning)} ·
                  위험 {formatDuration(telemetry.alerts?.thresholds?.avgAlertDurationMs?.critical)}
                </dd>
              </div>
              <div>
                <dt>자동 교체 소요</dt>
                <dd>
                  주의{' '}
                  {formatDuration(telemetry.alerts?.thresholds?.avgRotationDurationMs?.warning)} ·
                  위험{' '}
                  {formatDuration(telemetry.alerts?.thresholds?.avgRotationDurationMs?.critical)}
                </dd>
              </div>
              <div>
                <dt>연속 재시도 횟수</dt>
                <dd>
                  주의 {telemetry.alerts?.thresholds?.attemptsWithoutSuccess?.warning ?? '—'}회 ·
                  위험 {telemetry.alerts?.thresholds?.attemptsWithoutSuccess?.critical ?? '—'}회
                </dd>
              </div>
              <div>
                <dt>업로드 연속 실패</dt>
                <dd>
                  주의 ≥{' '}
                  {formatNumber(
                    telemetry.alerts?.thresholds?.timelineUploadFailureStreak?.warning ?? null
                  )}
                  회 · 위험 ≥{' '}
                  {formatNumber(
                    telemetry.alerts?.thresholds?.timelineUploadFailureStreak?.critical ?? null
                  )}
                  회
                </dd>
              </div>
              <div>
                ...
                <dd>
                  주의 ≥{' '}
                  {formatNumber(
                    telemetry.alerts?.thresholds?.timelineUploadStaleHours?.warning ?? null,
                    { digits: 0 }
                  )}
                  시간 · 위험 ≥{' '}
                  {formatNumber(
                    telemetry.alerts?.thresholds?.timelineUploadStaleHours?.critical ?? null,
                    { digits: 0 }
                  )}
                  시간
                </dd>
              ...
              <div>
                ...
                <dd>
                  전체 주의 ≤{' '}
                  {formatPercent(
                    telemetry.alerts?.thresholds?.docLinkAttachmentRate?.warning ?? null,
                    0
                  )}{' '}
                  · 위험 ≤{' '}
                  {formatPercent(
                    telemetry.alerts?.thresholds?.docLinkAttachmentRate?.critical ?? null,
                    0
                  )}{' '}
                  / 최근 주의 ≤{' '}
                  {formatPercent(
                    telemetry.alerts?.thresholds?.lastDocLinkAttachmentRate?.warning ?? null,
                    0
                  )}{' '}
                  · 위험 ≤{' '}
                  {formatPercent(
                    telemetry.alerts?.thresholds?.lastDocLinkAttachmentRate?.critical ?? null,
                    0
                  )}
                </dd>
              </div>
          ...
          <section className={styles.panel}>
            ...
            {timelineUploadAlert.issues?.length ? (
              <IssueList issues={timelineUploadAlert.issues} />
            ) : null}
          ...
          <section className={styles.panel}>
            <header className={styles.panelHeader}>
              ...
              <div className={styles.auditStats}>
                ...
                {thresholdAudit?.recentWindowHours ? (
                  <span>
                    최근 {thresholdAudit.recentWindowHours}시간{' '}
                    {formatNumber(thresholdAudit?.recentCount ?? 0)}회
                  </span>
                ) : null}
              </div>
            </header>
          ...
          <section className={styles.panel}>
            ...
            <p className={styles.caption}>
              최근 랭크 전투 로그를 분석해 단어별 사용량, 승률 상관관계, OP 문장을 티어로
              정리했습니다.
            </p>
            <div className={styles.languageControls}>
              ...
              <label className={styles.languageControl}>
                ...
                <select
                  ...
                  disabled={
                    languageFiltersLoading || (languageFilterOptions?.games || []).length === 0
                  }
                >
            ...
            <div className={styles.languageFavorites}>
              <div className={styles.languageFavoriteToolbar}>
                ...
                <div className={styles.languageFavoriteButtons}>
                  <button
                    type="button"
                    className={styles.languageFavoritePrimaryButton}
                  ...
                  {filterFavorites.map(favorite => (
                    <li key={favorite.id} className={styles.languageFavoriteItem}>
                      ...
                      >
                        ...
                        <span className={styles.languageFavoriteSummary}>
            ...
            {languageError ? (
              ...
              <>
                <div className={styles.languageStats}>
                  ...
                  <span>
                    기준 승률{' '}
                    {languageInsights.baseline?.winRate !== null &&
                    languageInsights.baseline?.winRate !== undefined
                ...
                {languageInsights.meta?.missingTable && (
                  <p className={styles.languageNotice}>
                    참고: 스테이징/로컬 환경에서 `rank_battle_logs` 테이블이 아직 생성되지 않아 샘플
                    없이 빈 카드가 표시됩니다. DDL을 적용한 뒤 새로고침해 주세요.
                  </p>
                )}
