...
async function claimDropInSlot({ supabase, room, slot, entry }) {
  ...
  if (room?.id) {
    ...
    await withTable(supabase, 'rank_rooms', table =>
      supabase.from(table).update(payload).eq('id', room.id)
    );
  }
...
export function extractMatchingToggles(gameRow, rules = {}) {
  ...
  for (const key of DROP_IN_RULE_KEYS) {
    ...
    if (typeof candidate === 'string') {
      ...
      if (
        ['allow', 'allow-drop-in', 'enabled', 'enable', 'on', 'true', 'yes', '1'].includes(
          normalized
        )
      ) {
        dropInEnabled = true;
      ...
      if (
        ['forbid', 'disabled', 'disable', 'off', 'false', 'no', '0', 'ban'].includes(normalized)
      ) {
        dropInEnabled = false;
        break;
      }
...
export async function loadMatchingResources({
  supabase,
  gameId,
  mode,
  realtimeEnabled,
...
export function buildCandidateSample({
  ...
  const ownersInQueue = new Set(
    baseQueue.map(row => row?.owner_id || row?.ownerId).filter(Boolean)
  );
  ...
  const totalSlots = Array.isArray(roles)
    ? roles.reduce(
        (acc, role) =>
          acc + Math.max(0, Number(role?.slot_count ?? role?.slotCount ?? role?.slots ?? 0) || 0),
        0
      )
    : 0;
  ...
  const allowDuplicateOwners =
    duplicateOwnerCandidates.length > 0 && requiredStandins > uniqueOwnerCandidates.length;
...
export async function findRealtimeDropInTarget({
  ...
  for (const candidate of candidates) {
    const claim = await claimDropInSlot({
      supabase,
      room: candidate.room,
      slot: candidate.slot,
    ...
    const roleName =
      normalizeRoleName(candidate.slot.role) || normalizeRoleName(candidate.entry.role);
    ...
    if (allOpenSlotsForRole.length > 1 && viableEntries.length > 1) {
      for (const otherEntry of viableEntries) {
        ...
        // Claim this slot
        const claimResult = await claimRealtimeSlot({
          supabase,
        ...
        additionalMembers.push({
          ...
          slotIndex: Number.isFinite(Number(availableSlot.slot_index))
            ? Number(availableSlot.slot_index)
            : 0,
        });
      }
    }
    ...
    const assignments = [
      {
        ...
        groupKey: `drop_in:${
          ...
          normalizeId(claim.slot.id) ||
          normalizeId(candidate.room.id)
        }`,
    ...
    return {
      ...
      dropInTarget: {
        ...
        averageScore:
          candidate.stats.roleAverages.get(normalizeRoleName(candidate.slot.role)) ??
          candidate.stats.overallAverage ??
          null,
        occupantCount: candidate.stats.occupantCount,
        openSlotsBefore:
          (candidate.stats.openSlotCountByRole || candidate.stats.openSlotsByRole)?.get(
            normalizeRoleName(candidate.slot.role)
          ) || 1,
        claimedAt: claim.now,
      },
      meta: attemptMeta,
