...
function buildMatchingMetadata(payload: Record<string, unknown>) {
  ...
  return {
    matching: {
      source:
        safeString(matching.source) ??
        safeString(payload.source) ??
        'edge:rank-match-timeline',
      matchType:
      ...
      storedAt,
      mode:
        safeString(matching.mode) ?? safeString(payload.mode) ?? null,
      turnTimer:
...
serve(async (req) => {
  ...
  const context = {
    ...
    sessionLabel:
      safeString(payload.session_label) ??
      safeString(payload.sessionLabel) ??
      null,
    sessionCreatedAt:
      safeString(payload.session_created_at) ??
      safeString(payload.sessionCreatedAt) ??
      null,
  }
