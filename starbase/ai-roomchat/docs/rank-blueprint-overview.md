...
- [임계값 감사 타임라인](rank-api-key-cool다운-monitoring.md#4-telemetry-리포트--대시보드): `thresholdAudit.timeline`이 최근 14일 변경 빈도를 버킷 단위로 제공하고, 대시보드 타임라인 그래프가 오늘 이벤트를 강조해 추세를 시각화합니다.
...
| 운영 가드 | 진행 중 | 82% | 감사 로그 기반 백오프 스케줄러, Slack ETA 안내, 임계값 변경 카드·감사 로그 패널, 감사 타임라인 그래프까지 연동돼 운영자가 재시도·임계값 추세를 한 화면에서 추적할 수 있습니다. |

**총 진행률(단계별 동일 가중치)**: 약 **62%**
