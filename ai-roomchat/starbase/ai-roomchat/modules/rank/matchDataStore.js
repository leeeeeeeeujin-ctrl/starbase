...
function sanitizeHistoryTurn(turn, index) {
  ...
  const metadata =
    turn.metadata && typeof turn.metadata === 'object' ? safeClone(turn.metadata) : null;
...
function sanitizeSessionHistoryPayload(patch, previous) {
  ...
  const base = previous
    ? safeClone(previous) || createEmptySessionHistory()
  ...
  if (patch && typeof patch === 'object') {
    ...
    if (patch.publicCount !== undefined) {
      ...
      next.publicCount =
        Number.isFinite(publicCount) && publicCount >= 0 ? Math.floor(publicCount) : 0;
    }

    if (patch.hiddenCount !== undefined) {
      const hiddenCount = Number(patch.hiddenCount);
      next.hiddenCount =
        Number.isFinite(hiddenCount) && hiddenCount >= 0 ? Math.floor(hiddenCount) : 0;
    }
      ...
      return {
        ...
        slots:
          Number.isFinite(slotCount) && slotCount >= 0
            ? slotCount
            : sanitizedSlots.length || members.length,
        members,
...
function sanitizeSlotTemplatePayload(payload, previous) {
  ...
  const base = previous
    ? safeClone(previous) || createEmptySlotTemplate()
...
function sanitizeSessionMetaPatch(patch, previous) {
  ...
  const base = previous
    ? safeClone(previous) || createEmptySessionMeta()
...
function sanitizeTurnStatePatch(patch, previous) {
  ...
  const base =
    previous && typeof previous === 'object'
      ? safeClone(previous) || createEmptyTurnState()
  ...
  if (patch && typeof patch === 'object') {
    ...
    if (patch.dropInBonusTurn !== undefined) {
      ...
      next.dropInBonusTurn =
        Number.isFinite(bonusTurn) && bonusTurn >= 0 ? Math.floor(bonusTurn) : 0;
    }
    if (patch.source !== undefined) {
      next.source = typeof patch.source === 'string' ? patch.source.trim() : next.source || '';
    }
...
function buildAsyncFillSnapshot({
  ...
  const hostRoleSeats = orderedRoster.filter(entry => normalizeRoleKey(entry.role) === hostRoleKey);
  ...
  const pendingSeats = assignedSeats.filter(entry => !entry.ownerId).map(entry => entry.slotIndex);
  ...
  const explicitUpdatedAt =
    payload && typeof payload === 'object'
      ? normalizeTimestampInput(
          payload.updatedAt ?? payload.updated_at ?? payload.updated_at_ms ?? null
        )
      : null;
