...
const defaultTurn = () => ({ idx: 0, role: 'system', content: 'ready', createdAt: new Date().toISOString() })
...
class MockGameServer {
  ...
  listGames() {
    return Array.from(this.games.values()).map((g) => ({ id: g.id, createdAt: g.createdAt, updatedAt: g.updatedAt }))
  }
  ...
  joinGame(gameId, { ownerId, heroId, role }) {
    ...
    g.slots.push({ slotIndex, ownerId: String(ownerId || `owner-${slotIndex}`), heroId: String(heroId || ''), role: String(role || 'player'), ready: true })
  ...
  addTurn(gameId, { role, content }) {
    ...
    g.turns.push({ idx, role: String(role || 'player'), content: String(content || ''), createdAt: new Date().toISOString() })
    g.updatedAt = Date.now()
    return this.getSnapshot(gameId)
  }
  ...
  getSnapshot(gameId) {
    ...
    return {
      ...
      session: g.session,
      slots: g.slots,
      history: { sessionId: g.session.id, turns: g.turns, totalCount: g.turns.length, updatedAt: g.updatedAt },
