...
function AIAssistPanel({ promptId }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 8 }}>
        <button onClick={runAssist} disabled={loading}>
          {loading ? 'Running…' : 'Run AI Assist'}
        </button>
        <Link href={`/prompts/${encodeURIComponent(promptId)}/edit`}>
          <a style={{ marginLeft: 12 }}>Back to editor</a>
        </Link>
      </div>
      <div style={{ marginTop: 16 }}>
        <h4>AI Response</h4>
        <pre style={{ whiteSpace: 'pre-wrap' }}>
          {response ? response.text : <em>No response yet</em>}
        </pre>
      </div>
    </div>
  );
