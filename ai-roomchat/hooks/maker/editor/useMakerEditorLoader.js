...
function buildVersionAlert(slotRows = []) {
  ...
  slotRows.forEach((slot, index) => {
    ...
    if (
      globalInfo.hadEntries &&
      (globalInfo.legacyStructure || globalInfo.version !== VARIABLE_RULES_VERSION)
    ) {
    ...
    if (
      localInfo.hadEntries &&
      (localInfo.legacyStructure || localInfo.version !== VARIABLE_RULES_VERSION)
    ) {
