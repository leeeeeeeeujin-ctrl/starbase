...
import { buildVariableRules, buildSystemPromptFromChecklist, safeStr } from '@/lib/promptEngine';
...
const DEFAULT_RULE_GUIDANCE_ENTRIES = [
  {
    text: '- 응답의 마지막 여덟 줄 중 위 다섯 줄은 공란으로 남겨 두고, 맨 아래 세 줄만 다음 형식으로 채워라.',
    matchers: [/마지막/, /(다섯|5)[^\s]*\s*줄/, /(공란|공백)/],
  },
  {
    text: '- 마지막에서 세 번째 줄에는 이번 턴에서 독보적인 활약을 펼친 캐릭터 이름을 적되, 적합한 인물이 없으면 비워 둔다.',
    matchers: [/마지막.*줄/],
  },
  {
    text: '- 마지막에서 두 번째 줄에는 활성화된 변수명을 공백으로 구분해 적고, 없다면 "무"만 적어라.',
    matchers: [/마지막/, /(둘|두).*줄/, /(변수|변수명)/],
  },
  {
    text: '- 마지막 줄에는 해당 턴의 판정을 캐릭터 이름과 함께 "승리", "패배", "탈락" 중 하나로 선언하고, 판정이 없다면 "무"만 적어라.',
  ...
  const processEntries = entries => {
    ...
    if (Array.isArray(entries)) {
      entries.forEach(entry => {
        ...
        const key = entry.key ?? entry.id ?? entry.name ?? entry.option ?? entry.label ?? null;
        if (!key) return;
...
function buildVariableStateLines(names = [], scopeLabel = '전역') {
  ...
  // Use Korean '무' when no variables are active to align with test expectations and keep deterministic output
...
function normalizeOwnerId(participant, hero) {
  const owner =
    participant?.owner_id ?? participant?.ownerId ?? hero?.owner_id ?? hero?.ownerId ?? null;
...
function buildRuleSections({ game, node, activeGlobalNames = [], activeLocalNames = [] } = {}) {
  const sections = {
    baseRules: [],
    globalVariables: [],
    localVariables: [],
  ...
  DEFAULT_RULE_GUIDANCE_ENTRIES.forEach(({ text, matchers = [] }) => {
    ...
    const hasSimilarLine =
      Array.isArray(matchers) && matchers.length
