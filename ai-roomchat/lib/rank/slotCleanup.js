...
// intentionally isolated from API handlers so they can be reused by internal schedulers,
...
function evaluateRelease({ slot, participant, queueEntry, nowMs, timeoutMs }) {
  ...
  if (TIMEOUT_CANDIDATE_STATUSES.has(status) && staleSlot && staleParticipant && staleQueue) {
  ...
  if (statusesToUpdate.length) {
    ...
    for (const [status, ids] of grouped.entries()) {
      const statusResult = await withTable(supabaseClient, 'rank_participants', table =>
        supabaseClient.from(table).update({ status, updated_at: nowIso }).in('id', ids)
      );
