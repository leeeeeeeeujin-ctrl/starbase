...
- 홈(`/`)은 타이틀 → 로스터 → 캐릭터(로비) 순서를 먼저 강조한다. CTA 아래에 방 검색 대체 경로로서 Arena 안내 배너만
  삽입해 기본 UI는 유지하면서도 큐 → 준비 → 본게임 흐름을 확인할 수 있다.
- `ArcadeLayout` 헤더 역시 "기본 흐름" 그룹을 가장 위에 두고, Arena·운영·레거시 링크를 보조 섹션으로 내려서 원하는 순서대로 탐색할 수 있다. 운영/개발자는 큐 티켓·세션 ID 중심의 새 모델과 기존 방 데이터 모델을 병행 확인할 수 있다.
