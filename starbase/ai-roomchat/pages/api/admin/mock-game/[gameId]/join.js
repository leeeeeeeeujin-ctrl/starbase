...
import { getMockGameServer } from '@/lib/mockGameServer';
...
export default async function handler(req, res) {
  ...
  const { ownerId, heroId, role } = req.body || {};
  const server = getMockGameServer();
  try {
    const snapshot = server.joinGame(gameId, { ownerId, heroId, role });
