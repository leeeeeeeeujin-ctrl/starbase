...
export default async function handler(req, res) {
  ...
  if (!authorized && matchInstanceId) {
    try {
      const { data: rosterRow } = await withTableQuery(supabaseAdmin, 'rank_match_roster', from =>
        from
  ...
  if (!authorized && gameId) {
    try {
      const { data: participantRow } = await withTableQuery(
        ...
        'rank_participants',
        from =>
          from.select('id, owner_id').eq('game_id', gameId).eq('owner_id', userId).maybeSingle()
      );
      if (participantRow) {
        authorized = true;
