  ...
  async generateCode(prompt, context = {}) {
    ...
    try {
      ...
      if (this.services.analytics) {
        ...
        this.services.analytics.trackError(error, { context: 'ai_code_generation', prompt: prompt.substring(0, 100) })
      }
...
위 게임 컨텍스트를 고려하여 요청을 처리해주세요.`
  ...
  async updateScore(scoreChange, reason = 'game_action') {
    try {
      ...
      this.log('🎯 점수 업데이트:', oldScore, '->', this.gameState.score, `(${scoreChange > 0 ? '+' : ''}${scoreChange})`)
