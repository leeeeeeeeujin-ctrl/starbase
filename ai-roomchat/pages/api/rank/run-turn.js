...
export default async function handler(req, res) {
  try {
    ...
    const { data: session, error: sessionError } = await withTableQuery(
      supabaseAdmin,
      'rank_sessions',
      from => from.select('id, owner_id, game_id, status, turn').eq('id', sessionId).maybeSingle()
    );
    ...
    const { error: updateError } = await withTableQuery(supabaseAdmin, 'rank_sessions', from =>
      from.update({ updated_at: now, turn: nextTurnNumber }).eq('id', sessionId)
    );
    ...
    console.error('[run-turn] unexpected error:', error);
    const message =
