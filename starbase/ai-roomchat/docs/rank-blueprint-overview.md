...
## 5. 최근 진행 스냅샷 (2025-11-13)
- 관리자 모니터링 포털을 점검해 랜딩 히어로·공지·튜토리얼을 코드 배포 없이 교체하려면 경량 CMS가 필요하다는 결론을 내렸습니다.
- 이를 반영해 "운영 모니터링·콘텐츠 확장" 단계를 추가하고, 콘텐츠 허브·승인 워크플로·배포 이력 테이블 요구사항을 정리했습니다.
- 히어로/문서/대시보드 UI가 남은 단계를 강조하도록 검수하여 진행도 카드와 다음 액션 카드가 동일한 데이터를 반영함을 확인했습니다.

<!-- blueprint-next-actions:start -->
## 6. 다음 액션 스냅샷

<!-- next-actions-status:start -->
> _2025-11-13 기준 자동 생성된 기한 알림._
...
**총 진행률(단계별 동일 가중치)**: 약 **86%**

<!-- blueprint-progress:end -->

<!-- blueprint-remaining:start -->
### 8. 남은 청사진 핵심 작업 (2025-11-13 기준)
| 단계 | 남은 핵심 작업 | 현재 진행률 |
| --- | --- | --- |
...
랜딩 히어로는 이 표를 기반으로 전체 진행도 카드(약 86%, 남은 단계 1개)를 노출해 남은 작업을 즉시 확인할 수 있습니다.【F:data/rankBlueprintProgress.json†L1-L37】【F:pages/index.js†L8-L210】【F:styles/Home.module.css†L1-L360】
