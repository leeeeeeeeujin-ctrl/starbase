...
describe('device runner integration (in-process mock)', () => {
  ...
  test('full device-run -> verify -> save', async () => {
    ...
    const stored = saveRun({ prompt_id: prompt.id, prompt_version: prompt.version, input, rendered_prompt: rendered, provider: 'client', provider_response: verification.sanitizedResponse || providerResponse, status: verification.verified ? 'ok' : 'unverified' })
