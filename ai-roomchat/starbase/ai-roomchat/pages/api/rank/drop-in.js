...
import { extractMatchingToggles, findRealtimeDropInTarget, loadMatchingResources } from '@/lib/rank/matchingPipeline'
...
export default async function handler(req, res) {
  ...
  const debugEnabled = String(process.env.RANK_DEBUG_CONSOLE || '').trim().toLowerCase() === 'true'
  ...
  try {
    ...
    if (toggles.realtimeEnabled && toggles.dropInEnabled) {
      ...
      const dropIn = await findRealtimeDropInTarget({ supabase, gameId, mode, roles, queue, rules })
      ...
      if (!dropIn || !dropIn.ready) {
        ...
        return res.status(200).json({ ready: false, error: dropIn?.meta?.reason || 'no_realtime_drop_in_target', meta: dropIn?.meta || null, requestId })
      }
    ...
    if (!result.ready) {
      ...
      return res.status(200).json({ ready: false, assignments: result.assignments || [], totalSlots: result.totalSlots, error: result.error || null, requestId })
    }

    const matchCode = `dropin_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,8)}`
    await markAssignmentsMatched(supabase, { assignments: result.assignments, gameId, mode, matchCode })
    ...
    await logStage({
      ...
      score_window: result.maxWindow || null,
      metadata: { mode, realtime: false, assignments: buildAssignmentSummary(result.assignments || []) },
    })

    return res.status(200).json({ ready: true, assignments: result.assignments, totalSlots: result.totalSlots, matchCode, matchType: 'offline_drop_in', requestId })
  } catch (error) {
    console.error('drop-in failed:', { requestId, error })
    await logStage({ stage: 'drop_in', status: 'error', metadata: { error: error?.message || String(error) } })
