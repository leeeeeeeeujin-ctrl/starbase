...
| 2 | 히어로/담당자 즐겨찾기와 Slack 알림 조건 저장을 지원해 스택 그래프·다이제스트에서 우선 모니터링 대상을 고정하고 채널별 구독 옵션을 노출합니다. | 프론트 (라라) | 2025-12-02 | P1 (오디오 튜닝) | 프론트 3일 (1명·UI/데이터) |
...
| UI·오디오 완성 | 진행 중 | 97% | 메인 룸 히어로 패널이 브금 자동 재생·컨트롤을 제공하고, StartClient 로그 보드는 섹션별 축약/펼치기 토글·검색 입력에 더해 검색어 하이라이트와 액션/주역/태그 다중 필터 칩을 갖춰 긴 히스토리에서도 필요한 카드만 빠르게 추리며, 모바일에선 히스토리 패널이 세로로 재배치되고 데스크톱에선 다단 그리드로 확장돼 전투/AI/플레이어 히스토리를 화면 크기에 맞춰 보여줍니다. 브금 재생목록 전환, EQ/리버브/컴프레서 토글, 프리셋 칩까지 연동된 상태에서 선택한 트랙과 이펙트는 사용자·운영자별로 저장되고 Supabase 로그로 남아 재진입 시 자동 복원됩니다. 관리자 포털에는 Owner/히어로/이벤트 유형·검색어 필터와 기간 스위치를 갖춘 오디오 이벤트 패널이 주간 추이 그래프·증감 배지를 포함해 CSV·Slack 다이제스트와 함께 최신 변경 로그를 한눈에 검토할 수 있도록 확장됐으며, 히어로/담당자 스택 그래프는 상위 3·5·전체 토글과 스크롤 범례를 지원해 분포가 많아져도 한눈에 비교할 수 있습니다. |
