...
## 구현 진행 현황 업데이트
- **진행률**: 90% – 비실시간/난입 이벤트까지 하나의 타임라인 포맷으로 통합하고, 재사용 가능한 관전 카드 컴포넌트를 마련해 Phase 3 후반 작업이 안정화 단계에 들어섰습니다.
- **완료 사항**:
  - `useStartClientEngine`이 `recordTimelineEvents`를 통해 경고·대역뿐 아니라 난입 합류, 시간 만료, 80% 합의 이벤트를 동일 스키마로 기록·브로드캐스트하며 비실시간 모드와도 로그 포맷을 공유합니다.
...
- **다음 단계 메모**:
  - `asyncSessionManager` 도입으로 비실시간 대역 호출/쿨다운 교체 흐름을 정리하고, 타임라인 이벤트에 API 키 교체 이유를 첨부하기.
  - 베틀로그 재생·관전 화면에서 `TimelineSection`을 활용하도록 데이터 파이프라인을 확장하고, 검색/필터 인터랙션을 추가하기.
  - 난입 큐(`dropInQueueService`) 구현 시 타임라인 이벤트에 역할군 교체 이력과 승수 메타데이터를 함께 남길 수 있도록 스펙을 확정하기.

---

느낀 점: 모드와 무관하게 같은 타임라인 이벤트 객체를 쓰니 난입·시간 초과·합의 흐름을 훨씬 설명하기 쉬워졌고, 관전 UI도 손쉽게 확장할 수 있다는 자신감이 생겼습니다.
추가로 필요한 점: 비실시간 세션 매니저와 난입 큐를 연결해 이벤트 로그에 대역 교체 사유, 후속 재배치 정보를 자동 첨부할 수 있도록 백엔드·프런트 스펙을 더 구체화해야 합니다.
진행사항: 드롭인 감지·자동 진행·합의 완료 이벤트를 통합 포맷으로 로깅하고, 재사용형 `TimelineSection`을 도입해 StartClient와 향후 베틀로그 재생 화면을 잇는 발판을 마련했습니다.
