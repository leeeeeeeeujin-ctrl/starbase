...
function normaliseWeeklyBuckets(buckets = [], { weeks = 12, now = new Date() } = {}) {
  ...
  if (Array.isArray(buckets)) {
    for (const bucket of buckets) {
      ...
      map.set(key, {
        eventCount: Number.isFinite(bucket.eventCount)
          ? bucket.eventCount
          : Number(bucket.eventCount) || 0,
        uniqueOwners: Number.isFinite(bucket.uniqueOwners)
          ? bucket.uniqueOwners
          : Number(bucket.uniqueOwners) || 0,
...
function buildStackedTrend(
  ...
  const legend = dimensionEntries.map(([dimensionId, total], index) => ({
    ...
    total,
    color:
      palette[index % palette.length] || palette[palette.length - 1] || 'rgba(148, 163, 184, 0.8)',
  ...
  const bars = buckets.map(bucket => {
    ...
    const segments = legend.map(entry => {
      const value = entry.sourceIds.reduce(
        (sum, sourceId) => sum + (weekMap.get(sourceId) || 0),
        0
      );
    ...
    const tooltipLines = segments
      ...
      .map(
        segment =>
          `${segment.label}: ${formatNumber(segment.count)}건 (${Math.round(segment.percentage)}%)`
      )
      .join('\n');
...
function formatTrendDelta(summary) {
  ...
  const percent = Number.isFinite(summary.deltaPercent)
    ? Math.abs(summary.deltaPercent).toFixed(1)
    : '0.0';
...
export default function AudioEventMonitor() {
  ...
  const startEditFavorite = useCallback(
    favorite => {
      if (!favorite) {
        resetFavoriteForm();
        return;
  ...
  const startEditSubscription = useCallback(
    subscription => {
      if (!subscription) {
  ...
  const handleSaveFavorite = useCallback(async () => {
    ...
    try {
      ...
      resetFavoriteForm();
      setRulesFeedback(
        editingFavoriteId ? '즐겨찾기를 업데이트했습니다.' : '즐겨찾기를 저장했습니다.'
      );
    } catch (err) {
  ...
  const handleSaveSubscription = useCallback(async () => {
    ...
    try {
      ...
      resetSubscriptionForm();
      setRulesFeedback(
        editingSubscriptionId ? '구독 조건을 업데이트했습니다.' : '구독 조건을 저장했습니다.'
      );
    } catch (err) {
  ...
  const trendSummary = useMemo(
    () => summariseTrend(normalizedTrendBuckets),
  ...
  const trendBreakdown = useMemo(
    () => ({
      hero: normaliseBreakdown(trendData?.breakdown?.hero || [], {
        fallbackLabel: '히어로 미지정',
      }),
      owner: normaliseBreakdown(trendData?.breakdown?.owner || [], {
  ...
  useEffect(() => {
    ...
    const appliedAt =
      stored.appliedAt && !Number.isNaN(Date.parse(stored.appliedAt))
        ? stored.appliedAt
    ...
    <section className={styles.audioEventsSection}>
      ...
      {eventsMeta.missingTable ? (
        <p className={styles.audioEventsNotice}>
          <code>rank_audio_events</code> 테이블이 아직 생성되지 않아 로그가 비어 있습니다. Supabase
          마이그레이션을 적용하고 관리자 페이지를 새로고침해주세요.
        </p>
      ) : null}
      ...
      <div className={styles.audioEventsTrend}>
          ...
          <div className={styles.audioEventsTrendHeading}>
            ...
            <div className={styles.audioEventsTrendModes}>
              ...
              <button
                ...
                disabled={
                  trendLoading || trendUnavailable || !heroBreakdownAvailable || !hasHeroBreakdown
                }
                title={
                  trendUnavailable
              ...
              <button
                ...
                disabled={
                  trendLoading || trendUnavailable || !ownerBreakdownAvailable || !hasOwnerBreakdown
                }
                title={
                  trendUnavailable
                    ? '주간 추이 함수가 준비되지 않아 분포를 계산할 수 없습니다.'
        ...
        {trendUnavailable ? (
          <p className={styles.audioEventsTrendNotice}>
            <code>rank_audio_events_weekly_trend</code> RPC가 아직 Supabase에 배포되지 않아 주간
            추이를 계산할 수 없습니다. 운영 스키마에 함수를 적용한 뒤 새로고침해주세요.
          </p>
        ) : (
          <>
            ...
            <div
              className={styles.audioEventsTrendChart}
              role="img"
              aria-label="오디오 이벤트 주간 추이"
            >
                    ...
                    <div className={styles.audioEventsTrendBarTrack} title={bar.tooltip}>
                      {usingStackedTrend ? (
                        ...
                        >
                          ...
                          {bar.total > 0 ? (
                            <span className={styles.audioEventsTrendBarStackValue}>
                        ...
                        >
                          {bar.eventCount > 0 ? (
                            <span className={styles.audioEventsTrendBarValue}>
                              {formatNumber(bar.eventCount)}
                            </span>
                          ) : null}
                        </div>
                      )}
            ...
            {usingStackedTrend && trendStackLegend.length ? (
              <>
                <div className={styles.audioEventsTrendLegend}>
                  {trendStackLegend.map(item => (
                    <span
                      ...
                      title={
                        item.sourceCount > 1
                          ? `${item.label} 포함 그룹 ${item.sourceCount}개`
                          : item.label
                      }
                    >
                      <span
                ...
                {trendStackLegendOverflowNote ? (
                  <p className={styles.audioEventsTrendLegendNote}>
                    {trendStackLegendOverflowNote}
                  </p>
                ) : null}
              </>
      ...
      {rulesFeedback ? (
        <p className={styles.audioEventsPreferenceFeedback}>{rulesFeedback}</p>
      ) : null}
                ...
                <li key={favorite.id} className={styles.audioEventsPreferenceItem}>
                  <div className={styles.audioEventsPreferenceItemText}>
                    ...
                    <span className={styles.audioEventsPreferenceMeta}>
                      {summariseFilters(favorite.filters)}
                    </span>
                    {favorite.updatedAt ? (
                      <span className={styles.audioEventsPreferenceMeta}>
          ...
          <form className={styles.audioEventsPreferenceForm} onSubmit={handleFavoriteSubmit}>
            ...
            <label className={styles.audioEventsPreferenceFormRow}>
              ...
              <input
                ...
                onChange={event =>
                  setFavoriteForm(prev => ({ ...prev, label: event.target.value }))
                }
            ...
            <label className={styles.audioEventsPreferenceFormRow}>
              ...
              <textarea
                ...
                onChange={event =>
                  setFavoriteForm(prev => ({ ...prev, notes: event.target.value }))
                }
            ...
            <label className={styles.audioEventsPreferenceFormRow}>
              ...
              <input
                ...
                onChange={event =>
                  setFavoriteForm(prev => ({ ...prev, sortOrder: event.target.value }))
                }
              />
            </label>
            ...
            <div className={styles.audioEventsPreferenceFormActions}>
              <button
                type="submit"
                className={styles.audioEventsPreferenceSubmit}
              ...
              {subscriptions.map(subscription => (
                <li key={subscription.id} className={styles.audioEventsPreferenceItem}>
                  <div className={styles.audioEventsPreferenceItemText}>
                    ...
                    <span className={styles.audioEventsPreferenceMeta}>
                      {summariseFilters(subscription.filters)}
                    </span>
                    <span className={styles.audioEventsPreferenceMeta}>
                      {summariseSlack(subscription.slack)}
                    </span>
          ...
          <form className={styles.audioEventsPreferenceForm} onSubmit={handleSubscriptionSubmit}>
            ...
            <label className={styles.audioEventsPreferenceFormRow}>
              ...
              <input
                ...
                onChange={event =>
                  setSubscriptionForm(prev => ({ ...prev, label: event.target.value }))
                }
            ...
            <label className={styles.audioEventsPreferenceFormRow}>
              ...
              <textarea
                ...
                onChange={event =>
                  setSubscriptionForm(prev => ({ ...prev, notes: event.target.value }))
                }
            ...
            <label className={styles.audioEventsPreferenceFormRow}>
              ...
              <input
                ...
                onChange={event =>
                  setSubscriptionForm(prev => ({ ...prev, channel: event.target.value }))
                }
            ...
            <label className={styles.audioEventsPreferenceFormRow}>
              ...
              <input
                ...
                onChange={event =>
                  setSubscriptionForm(prev => ({ ...prev, webhookKey: event.target.value }))
                }
            ...
            <label className={styles.audioEventsPreferenceFormRow}>
              ...
              <input
                ...
                onChange={event =>
                  setSubscriptionForm(prev => ({ ...prev, mention: event.target.value }))
                }
                placeholder="예: @ops"
              />
            </label>
            <div className={styles.audioEventsPreferenceGridRow}>
              <label className={styles.audioEventsPreferenceFormRow}>
                ...
                <input
                  ...
                  onChange={event =>
                    setSubscriptionForm(prev => ({ ...prev, minEvents: event.target.value }))
                  }
                />
              </label>
              <label className={styles.audioEventsPreferenceFormRow}>
                ...
                <input
                  ...
                  onChange={event =>
                    setSubscriptionForm(prev => ({ ...prev, lookbackWeeks: event.target.value }))
                  }
                />
              </label>
              <label className={styles.audioEventsPreferenceFormRow}>
                ...
                <input
                  ...
                  onChange={event =>
                    setSubscriptionForm(prev => ({ ...prev, sortOrder: event.target.value }))
                  }
                />
              </label>
            </div>
            <label className={styles.audioEventsPreferenceCheckboxRow}>
              <input
                ...
                onChange={event =>
                  setSubscriptionForm(prev => ({ ...prev, alwaysInclude: event.target.checked }))
                }
              />
            ...
            <label className={styles.audioEventsPreferenceCheckboxRow}>
              <input
                ...
                onChange={event =>
                  setSubscriptionForm(prev => ({ ...prev, notifyOnAnomaly: event.target.checked }))
                }
              />
              <span>급증·급락 감지 시 강조 표시</span>
            </label>
            <p className={styles.audioEventsPreferenceHelp}>
              현재 필터 기준으로 Slack 다이제스트와 관리자 패널 통계가 강조됩니다. Webhook 키는 환경
              변수와 매칭할 때 사용합니다.
            </p>
            <div className={styles.audioEventsPreferenceFormActions}>
              <button
