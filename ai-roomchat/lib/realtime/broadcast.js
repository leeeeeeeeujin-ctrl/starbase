...
function describeRankTurnStateEventsTopic(segments) {
  ...
  switch (qualifier) {
    ...
    case 'match':
      return [
        { table: 'rank_turn_state_events', filter: buildEqFilter('match_instance_id', value) },
      ];
    default:
...
function normaliseChangePayload(payload, topic, fallbackEvent) {
  const eventType =
    normaliseEvent(payload?.eventType || payload?.event || fallbackEvent) || 'INSERT';
  return {
    topic,
    eventType,
    event: eventType,
...
export function subscribeToBroadcastTopic(topic, handler, options = {}) {
  ...
  channel.subscribe((status, err) => {
    ...
    if (status === 'CHANNEL_ERROR') {
      console.error('[realtime] Postgres 변경 채널 구독 중 오류가 발생했습니다.', {
        topic,
        error: err || null,
      });
    }
...
export function subscribeToBroadcastTopics(topics, handler, options = {}) {
  ...
  const unsubscribers = uniqueTopics.map(topic =>
    subscribeToBroadcastTopic(topic, handler, options)
  );
