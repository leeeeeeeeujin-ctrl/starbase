...
export class FlexibleGameEngine {
  ...
  async endGame(reason = 'manual', finalData = {}) {
    ...
    if (
      ...
      this.gameState.score >= (this.config.endConditions.scoreThreshold || 1000)
    ) {
      result = 'win';
    ...
    try {
      ...
      if (gameData) {
        ...
        if (
          ...
          Object.keys(this.gameState.persistentData).length === 0
        ) {
  ...
  async savePersistentData() {
    try {
      ...
      localStorage.setItem(
        `game_${this.gameId}_persistent`,
        JSON.stringify(this.gameState.persistentData)
      );
