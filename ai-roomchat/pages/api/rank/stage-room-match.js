...
export default async function handler(req, res) {
  ...
  // Surface ready / suggestion metadata so clients (MatchReadyClient, CharacterPlayPanel)
  // can show the confirmation countdown and any suggested relaxations.
  const responseBody = {
  ...
  // Preserve backward compatibility: only include `ready` when true (clients
  // that don't expect the field won't break when it's absent). If explicit
  // false is desired in the future we can change the contract.
  if (rpcResult.data && rpcResult.data.ready === true) {
    responseBody.ready = true;
  }

  if (rpcResult.data && rpcResult.data.readyExpiresAt) {
    // RFC3339 / ISO timestamp expected by clients
    responseBody.ready_expires_at = rpcResult.data.readyExpiresAt;
  }

  if (rpcResult.data.suggestion) {
    // conservative suggestion payload generated by the matching service
    responseBody.suggestion = rpcResult.data.suggestion;
  }
