...
export default async function handler(req, res) {
  ...
  try {
    ...
    const extendedSelect = 'id, game_id, stage, status, score_window, mode_col:mode, drop_in, metadata, created_at'
    ...
    const usedExtended = Array.isArray(logsResult?.data) && logsResult?.table
    ...
    logs.forEach((log) => {
      ...
      const hourKey = new Date(Math.floor(timestamp / (60 * 60 * 1000)) * 60 * 60 * 1000).toISOString()
