...
export function mockSession(ownerId = 'user-1', heroId = 'hero-1', role = 'attacker', slotIndex = 0) {
  return {
    ownerId,
    heroId,
    role,
...
export function generateSampleRoster(count = 3) {
  return Array.from({ length: count }, (_, idx) => mockSession(`user-${idx+1}`, `hero-${idx+1}`, idx % 2 === 0 ? 'attacker' : 'defender', idx))
}
...
export function autoValidateRoster(roster) {
  ...
  return {
    ...
    hasDuplicates: roles.length !== roleSet.size || slotIndices.length !== slotSet.size || ownerIds.length !== ownerSet.size || heroIds.length !== heroSet.size,
  }
}
