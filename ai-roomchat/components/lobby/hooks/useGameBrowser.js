...
export default function useGameBrowser({ enabled, mode = 'public' } = {}) {
  ...
  const [availableSortOptions, setAvailableSortOptions] = useState(() =>
    getSortOptions({ includeMetrics: true })
  );
  ...
  useEffect(() => {
    ...
    async function loadGames() {
      ...
      if (
        ...
        table !== 'games' &&
        supportsGameMetrics &&
        isMissingColumnError(error, ['likes_count', 'play_count'])
      ) {
        if (!cancelled) {
      ...
      if (!cancelled && table === 'rank_games' && rows.length && mode !== 'owned') {
        ...
        const { data: seasonRows, error: seasonError } = await withTable(
          supabase,
          'rank_game_seasons',
          tableName => supabase.from(tableName).select('game_id, status').in('game_id', ids)
        );
  ...
  const joinSelectedGame = useCallback(
    async roleName => {
      ...
      if (storedOwnerId && String(storedOwnerId) !== String(viewerId)) {
        ...
        return {
          ok: false,
          error: '현재 계정과 다른 캐릭터가 선택되어 있어요. 다시 선택해 주세요.',
        };
      }
