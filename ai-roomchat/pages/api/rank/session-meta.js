...
function buildLegacyMetaRow(sessionId, metaPayload, skipColumns) {
  ...
  if (!skip.has('selected_time_limit_seconds')) {
    row.selected_time_limit_seconds = metaPayload.selected_time_limit_seconds ?? null;
  }
  ...
  if (!skip.has('drop_in_bonus_seconds')) {
    row.drop_in_bonus_seconds = metaPayload.drop_in_bonus_seconds ?? null;
  }
...
async function upsertMetaViaLegacyTable(client, sessionId, metaPayload, options = {}) {
  const initialSkip = Array.isArray(options.initialSkip) ? options.initialSkip : [];
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      if (!result.error) {
        const payload = Array.isArray(result.data) ? result.data[0] || null : result.data || null;
...
async function resolveSessionRoomId(
  sessionId,
  { fallbackRoomId = null, sessionOwnerId = null } = {}
) {
  ...
  try {
    const { data: roomRow, error: roomError } = await withTableQuery(
      ...
      from =>
        from
          .select('id')
          .eq('id', normalizedFallbackRoomId)
