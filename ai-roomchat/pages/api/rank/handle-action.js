// Action dispatcher API: validates, rate-limits, dispatches to actions
...
function checkRateLimit(key, allowUnlimited) {
  if (allowUnlimited) return { ok: true };
  const now = Date.now();
  const cur = bucket.get(key);
  if (!cur || cur.reset < now) {
    bucket.set(key, { reset: now + RATE_LIMIT_WINDOW_MS, count: 1 });
    return { ok: true };
  }
