...
export default function GameSimulator({ 
  ...
  // 게임 턴 실행
  const executeGameTurn = useCallback((currentSlot, activePlayer, allPlayers) => {
  ...
  const generateAIResponse = useCallback((slot, player, prompt) => {
    ...
    if (slotType === 'user_action' || slotType === 'user') {
      ...
      if (player.personality === 'aggressive') {
        return Math.random() > 0.3 ? '공격한다' : choices[Math.floor(Math.random() * choices.length)]
      } else if (player.personality === 'defensive') {
        return Math.random() > 0.3 ? '방어한다' : choices[Math.floor(Math.random() * choices.length)]
      } else {
  ...
  const determineNextSlot = useCallback((currentSlot, response, gameData) => {
    ...
    if (applicableBridge) {
      ...
      return gameData.slots.find(slot => 
        (slot.slot_no == toId) || (slot.slotNo == toId)
      )
    }

    // 브리지가 없으면 다음 순번 슬롯
    const currentIndex = gameData.slots.findIndex(slot => 
      (slot.slot_no == currentSlot.slot_no) || (slot.slotNo == currentSlot.slotNo)
    )
  ...
  // 시뮬레이션 종료
  const endSimulation = useCallback((reason) => {
    setSimulationState(prev => ({
      ...prev,
      running: false,
      logs: [...prev.logs, {
            ...
            {simulationState.logs.length === 0 ? (
              ...
              simulationState.logs.map((log, index) => (
                <div key={index} style={{
                  marginBottom: 16,
                  padding: 12,
                  background: log.type === 'system' ? 'rgba(59, 130, 246, 0.1)' : 'rgba(16, 185, 129, 0.1)',
                  borderRadius: 8,
