...
async function fetchHeroesByIds(heroIds) {
  ...
  const { data, error } = await withTable(supabase, 'heroes', table =>
    supabase.from(table).select('id,name,image_url,owner_id').in('id', unique)
  );
...
async function ensureCaches({ heroIdsToFetch, ownerIdsToFetch, heroCache, ownerCache }) {
...
export async function hydrateMessageList(
  rawMessages,
  {
    viewer = null,
    viewerHint = null,
