...
class GameContextManager {
  ...
  exportContext() {
    return {
      ...
      playerData: Object.fromEntries(
        Array.from(this.playerData.entries()).map(([id, data]) => [
          id,
  ...
  createPlayer(playerId, initialData = {}) {
    this.playerData.set(playerId, new Map(Object.entries({
      id: playerId,
      createdAt: Date.now(),
      score: 0,
      level: 1,
...
export const GameContext = {
  ...
  getPlayer: (id, key, defaultValue) => getGameContextManager().getPlayerData(id, key, defaultValue),
  
  // 변수
  setVar: (name, value, description) => getGameContextManager().setVariable(name, value, description),
