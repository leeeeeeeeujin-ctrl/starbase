...
const WALK_ROOT = path.resolve(ROOT, TARGET);
...
function shouldSkip(filePath) {
  const rel = path.relative(WALK_ROOT, filePath);
  if (!rel) return true;
...
function processFile(file) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const mNamed = line.match(
      /^\s*import\s+\{\s*([A-Za-z0-9_$]+)\s*\}\s+from\s+['"][^'\"]+['"];?\s*$/
    );
    if (mNamed) {
...
if (!fs.existsSync(WALK_ROOT)) {
  console.error('Target directory does not exist:', WALK_ROOT);
  process.exit(2);
}

console.log('Scanning for unused simple imports in', WALK_ROOT);
walk(WALK_ROOT);
