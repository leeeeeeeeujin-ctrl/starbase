...
export function buildCooldownAnalytics(rows = [], options = {}) {
  ...
  const providerFilter = options.provider && options.provider !== 'all' ? String(options.provider) : null
  const reasonFilter = options.reason && options.reason !== 'all' ? String(options.reason) : null

  const { start, end } = resolveRange({ start: options.start, end: options.end, range: options.range })
  ...
  for (const row of safeRows) {
    ...
    const recorded = normalizeDate(row.reported_at) || normalizeDate(row.recorded_at) || normalizeDate(row.updated_at) || normalizeDate(row.inserted_at)
    if (!recorded) {
      continue
    }
