...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!game || !heroes || heroes.length < 2) {
      pushStep('fetch-data', 'fail', { reason: 'not_enough_data', games: games?.length || 0, heroes: heroes?.length || 0 })
    ...
    pushStep('detail-initial', 'ok', { turn: detail1.session.turn, battles: (detail1.battles || []).length })
    ...
    pushStep('detail-after', 'ok', { turn: detail2.session.turn, battles: (detail2.battles || []).length })
    ...
    fs.writeFileSync(logPath, JSON.stringify({ ok: true, steps, timestamp: new Date().toISOString() }, null, 2))
    ...
    // best-effort cleanup
    if (sessionId) {
      try { await deleteRealSimulation(supabaseAdmin, sessionId) } catch {}
    }
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-')
    const logPath = path.join(logDir, `selftest-${timestamp}.json`)
    fs.writeFileSync(logPath, JSON.stringify({ ok: false, steps, timestamp: new Date().toISOString() }, null, 2))
