...
-- Ensure the rank_session_timeline_events table exists for drop-in timeline
-- broadcasts and upserts coming from `/api/rank/session-meta` and the edge
-- functions. Older databases may already define the table via supabase.sql so
-- we guard the creation with an existence check.
do $$
begin
  if not exists (
    select 1
...
-- Verifies that role definitions and slot templates remain in sync before
-- persisting registration payloads. The RPC raises descriptive errors on
-- mismatches so the API can return actionable responses to the client.
create or replace function public.verify_rank_roles_and_slots(
...
-- ---------------------------------------------------------------------------
-- fetch_rank_turn_state_events RPC
-- ---------------------------------------------------------------------------
...
begin
  if not exists (
    select 1
    from pg_proc
    where proname = 'fetch_rank_turn_state_events'
      and pronamespace = 'public'::regnamespace
  ) then
    -- The standalone SQL file creates the constraint guard as well; ensure the
    -- same check is present when applying the consolidated bundle.
    if not exists (
      select 1
      from pg_constraint
